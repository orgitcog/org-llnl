...
class DFAnalyzer:
    def __init__(
        self, file_pattern, load_fn=None, load_cols={}, load_data={}, metadata_cols={}
    ):
        ...
        if main_bag:
            ...
            # @Note: @ray, I added automatic time granularity finder here
            if self.conf.time_approximate and self.conf.time_granularity is None:
                logging.info("Calculating time granularity based on I/O events")
                io_filtered = self.events.query("phase == 2")[
                    ["pid", "tid", "ts", "dur"]
                ].rename(columns={"ts": "trange"})

                # Step 2: Sum durations grouped by (trange, pid, tid)
                summed = (
                    io_filtered.groupby(["trange", "pid", "tid"])["dur"]
                    .sum()
                    .reset_index()
                    .rename(columns={"dur": "total_dur"})
                )

                # Step 3: Max duration per trange
                maxed = (
                    summed.groupby("trange")["total_dur"]
                    .max()
                    .reset_index()
                    .rename(columns={"total_dur": "max_dur"})
                )

                # Step 4: Get the overall maximum of max_dur
                max_io_time = maxed["max_dur"].max().compute()
                self.conf.time_granularity = int(2 * max_io_time)
                logging.info(
                    f"Setting time granularity to {self.conf.time_granularity} microseconds based on I/O events"
                )
            ...
            self.events["trange"] = (
                self.events["ts"] // self.conf.time_granularity
    ...
    def _calculate_time(self):
        ...
        if self.conf.time_approximate:
            ...
            grouped_df["only_app_io"] = grouped_df[
                ...
                lambda s: s["app_io_time"] - s["compute_time"]
            ...
            grouped_df["only_app_compute"] = grouped_df[
                ...
                lambda s: s["compute_time"] - s["app_io_time"]
        ...
        logging.debug(
            f"Approximate {self.conf.time_approximate} total_time:{total_time}, {total_io_time}, {total_compute_time}, {total_app_io_time}, \
