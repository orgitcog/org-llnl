...
struct skywing::PubSubConverter<SirWalter>
{
    ...
    static pubsub_type convert(SirWalter sw)
    {
        return std::make_tuple(sw.meows, sw.purrs);
    }
...
TEST_CASE("PubSub Converter", "[mid]")
{
    ...
    using psNestedV_t =
        std::tuple<std::vector<std::uint32_t>, std::vector<std::int32_t>>;
    ...
    using psNestedSWV_t = std::tuple<std::vector<std::uint32_t>,
