...
// Type aliases for cheerio (cheerio 1.x doesn't export these types directly)
...
/**
 * Ancestor detection rule
 */
interface AncestorRule {
  /** CSS selector to match */
  selector: string;
  /** Function to extract context from matched element */
  extract: (el: CheerioElement, $: CheerioAPI) => string | null;
}
 ...
 */
const ANCESTOR_RULES: AncestorRule[] = [
  ...
  // Form: "in login form" / "in search form"
  {
    ...
    extract: (el, $) => {
      ...
      if (formName) return `${formName} form`;
      if (formRole === 'search') return 'search form';
      return 'form';
    }
  },
  ...
  // Navigation: "in main navigation" / "in sidebar navigation"
  {
    ...
    extract: (el, $) => {
      ...
      if (ariaLabel.toLowerCase().includes('main')) return 'main navigation';
      if (ariaLabel.toLowerCase().includes('sidebar')) return 'sidebar navigation';
      if (ariaLabel) return `${ariaLabel} navigation`;
      return 'navigation';
    }
  },
  ...
  // Tab panel: "in 'Settings' tab"
  {
    ...
    extract: (el, $) => {
      ...
      if (ariaLabel) return `'${ariaLabel}' tab`;
      if (labelledBy) {
        const tab = $(`#${labelledBy}`);
        const tabText = tab.text().trim();
        if (tabText) return `'${tabText}' tab`;
      }
      return 'tab panel';
    }
  },
 ...
 */
function getElementDescription(el: CheerioElement, $: CheerioAPI): string {
 ...
 */
function getTableContext(el: CheerioElement, $: CheerioAPI): string | null {
 ...
 */
function getListItemContext(el: CheerioElement, $: CheerioAPI): string | null {
 ...
 */
function walkAncestors(el: CheerioElement, $: CheerioAPI, maxDepth: number = 10): string[] {
  ...
  while (current.length > 0 && depth < maxDepth) {
    ...
    for (const rule of ANCESTOR_RULES) {
      if (current.is(rule.selector)) {
        const context = rule.extract(current as CheerioElement, $);
 ...
 */
export function findRecordedElement(html: string): ElementContext {
  ...
  const elementDesc = getElementDescription(recordedEl as CheerioElement, $);
  const ancestors = walkAncestors(recordedEl as CheerioElement, $);
 ...
 */
export function extractElementContext(htmlContent: string): ElementContext {
 ...
 */
export function formatElementContext(context: ElementContext): string {
  if (!context.found) {
    return 'element';
  }
