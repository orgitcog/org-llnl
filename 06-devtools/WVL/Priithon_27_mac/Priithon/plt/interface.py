...
def hold(state):
    ...
    if state in ['on','off','yes','no']:
        _active.client.hold = state  # 20110808 added .client
    else:
...
def plot(*data,**keywds):
    ...
    validate_active()
    if not _active.client.hold in ['on','yes']: # 20110808 added .client
        _active.client.line_list.data = [] # clear it out
        _active.client.image_list.data = [] # clear it out
    for i in lines:
        _active.client.line_list.append(i)
    _active.client.update()                
    return _active
...
def image(img,x=None,y=None,colormap = 'grey',scale='no'):
    ...
    if not _active.client.hold in ['on','yes']: # 20110808 added .client
        _active.client.line_list.data = [] # clear it out
        _active.client.image_list.data = [] # clear it out
        _active.client.image_list.append(image)
        try:
            axis('equal')
        except AttributeError:
