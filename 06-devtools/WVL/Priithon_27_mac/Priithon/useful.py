...
def timeIt(execStr, nReps=1, useTime=False):
    ...
    if nReps==1:
        t0 = useTime()
        exec execStr in fr.f_globals, fr.f_locals
        return useTime() - t0
    else:
...
def reload(*modules):
    """
    shortcut for non-verbose reloading of modules
    """
    import __builtin__
    for m in modules:
        __builtin__.reload(m)
...
def asUInt16(a, round=False, setMinTo0=False, rescaleToMax=False):
    ...
    if setMinTo0:
        a = a-a.min()
    if rescaleToMax:
        if rescaleToMax == 1:  # this includes `True`
            rescaleToMax = 65535
        a = a*(65535./a.max())
    if round:
        a = a+.5

    return N.asarray(a, N.uint16)
...
def fitAny(f, parmTuple0, data, fixed_mask=None, **leastsq_kwargs):
    ...
    if data.shape[-1] == 2:   # x,y tuples
        x,y = data.T
        yErr = 1
    elif data.shape[-1] == 3: # x,y,err tuples (weigthed fitting)
        x,y,yErr = data.T
...
def fitAny_leftSided(f, parmTuple0, data, xLeftSidedOfParm=0, fixed_mask=None, **leastsq_kwargs):
    """
    data should be list of (x,y)  tuples
    or (x,y,deltaY) [ weighted fitting: minimize (y(x)-x)/deltaY ]
    ...
    if fixed_mask is None or not any(fixed_mask):
        if yErr is 1:
            def func(p):
                return (x<=p[xLeftSidedOfParm]) *(f(p, x)-y)
        else:
            def func(p):
                return (x<=p[xLeftSidedOfParm]) *((f(p, x)-y) / yErr)
        ...
        def func(p):
            ...
            for par_i,par_fixed in enumerate(fixed_mask):
                if par_fixed:
                    ...
                    pp[par_i] = p[par_iHere]
                    par_iHere+=1
            return (x<=p[xLeftSidedOfParm]) *((f(pp, x)-y) / yErr)
...
try:
    ...
    def nd__center_of_mass(input, labels = None, index = None):
        ...
        if ll and len(index) == 1:
            r.shape = (1, -1)
        return r
    nd__center_of_mass.__doc__ = nd.center_of_mass.__doc__ + _U_nd_extra_doc_

    def nd__center_of_geometry(input, labels = None, index = None):
        """
        Calculate the geometric center of the values of an array at labels.

        Input is only used to determine the shape and dimentionality.
        ...
        This is adjusted from scipy.ndimage.center_of_mass, but returns ndarray and
        fixes return value type & shape
        for the cases  len(index) == 1 and len(index) == 0
        """
        ll = hasattr(index, "__len__")
        ...
        #ok in scipy 0.8 
        normalizer = nd.sum(1, labels, index)
        # for scipy .7 and same mem usage:
        #normalizer = nd.sum(N.ones(labels.shape), labels, index)
        grids = N.ogrid[[slice(0, i) for i in input.shape]]

        ##results = [sum(input * grids[dir].astype(float), labels, index) / normalizer for dir in range(input.ndim)]
        results = [nd.sum(grids[dir].astype(float), labels, index)  / normalizer for dir in range(input.ndim)]

        if N.isscalar(results[0]):
            return tuple(results)

        r = N.array( [tuple(v) for v in N.array(results).T] )
        ## r = N.array( nd.center_of_mass(input, labels, index) )
        if ll and len(index) == 1:
            r.shape = (1, -1)
        return r
    #nd__center_of_geometry.__doc__ = nd.center_of_mass.__doc__ + _U_nd_extra_doc_
    ...
    #         r = [ r ]
    #     return N.array( r )
    nd__maximum_position.__doc__ = nd.maximum_position.__doc__ + _U_nd_extra_doc_
    ...
    #         r = [ r ]
    #     return N.array( r )
    nd__minimum_position.__doc__ = nd.minimum_position.__doc__ + _U_nd_extra_doc_
    ...
    #         r = [ r ]
    #     return N.array( r )
    nd__sum.__doc__ = nd.sum.__doc__ + _U_nd_extra_doc_
...
def interpolate1d(x0, y, x=None):
    ...
    uses order 3 spline to do interpolation
...
def project_lowmem(a, mode='max', astype=None):
    """
    ...
    """
    if mode == "max":
        aProj = N.zeros(a.shape[1:], a.dtype)
        for aSlice in a:
            aProj = N.maximum(aProj, aSlice)
    elif mode == "mean":
        aProj = N.zeros(a.shape[1:], N.float64)
        for aSlice in a:
            aProj += aSlice
        aProj /= len(a)
    else:
        raise ValueError("mode must be one of 'mean' or 'max'.")
    if astype is not None:
        aProj = aProj.astype(astype)
    return aProj
...
def load(fn):
    ...
    meta={}
    meta['filename'] = fn
    meta['originLeftBottom'] = False # but see below

    if fn[-5:].lower() == ".fits":
        a = loadFits( fn )
        meta['fileformat'] = 'fits'
    elif fn[-4:].lower() == ".sif":
        a = loadSIF( fn )
        meta['fileformat'] = 'sif'
    elif fn[-4:].lower() == ".his":
        ...
        try:
            ...
            if fn[-4:].lower() == ".lsm":
                ...
                #a = loadImg(fn, iDelta=1)
                try:
                    import Image
                except:
                    from PIL import Image
                im = Image.open(fn)
                a = image2array(im)#, i0, iDelta, squeezeGreyRGB)
                meta['fileformat'] = im.format
            ...
            a = Mrc.bindFile(fn)
            meta['fileformat'] = 'mrc'
            meta['originLeftBottom'] = True

    from .ndarray_meta import ndarray_meta
    return ndarray_meta(a, meta=meta)
...
def loadImg_iterSec(fn, i0=0, iDelta=1, squeezeGreyRGB=True, secMax=100000):
    """
    iterator:
    ...
    import Image
    im = Image.open(fn)
    for i in xrange(i0, secMax, iDelta):
        try:
            im.seek(i)
...
def saveImg(arr, fn, forceMultipage=False, rgbOrder="rgba", format=None, **options):
    """
...
def loadSDT(fn):
    """
    load Becker & Hickl SDT files (.SDT)
    """
    from . import readerSDT
    return readerSDT.open_bh_sdt(fn)
...
def loadImageFromURL(url):
    ...
    #<addinfourl at 77154064 whose fp = <socket._fileobject object at 0x49829b0>>
    s = u.read()
    #import imghdr
    #imghdr.what(StringIO.StringIO(s))
    #'jpeg'
...
def loadTxt(fname, ndim=3, dtype=None, delimiter=None, skipNlines=0, 
    ...
    for ii,X in Xii.iteritems():
        ...
        ##20100312 why this magic !?
        #20100312 r,c = X.shape
        #20100312 if r==1 or c==1:
        #20100312     DEBUG_HERE()
        #20100312     X.shape = max((r,c)),
...
_getModules_cache={} # list of modules for each directory parsed

def getModules(return_dict=False, include_builtins=True, include_curdir=True, include_site_packages=True):
    """
    Return all modules available for import
    if `return_dict`:
        returns a mapping of modules name to path 
    else:
        return a sorted list of names

    Derived from code by Fredrik Lundh, 2005, 
    ...
    for p in path:
        try:
            ms = _getModules_cache[p]
        except KeyError:
            ms = _getModules2(p)
            _getModules_cache[p] = ms
        modules.update(ms)
...
def _getModules2(p):
    """
    Return a list of all modules available for import
    in `p`
    ...
    for f in os.listdir(p):
        ...
        if os.path.isfile(f):
            ...
            if m == "__init__":   # skip __init__.py modules
                continue
            ...
            fi = os.path.join(f, "__init__.py")
            if os.path.isfile(fi):
                modules[m] = fi # because __init__.py modules are skipped
                for mm, f in _getModules2(f).items():
                    modules[m + "." + mm] = f
    return modules
...
class deco_memoized(object):
   """Decorator that caches a function's return value each time it is called.
   If the function is called later with the same arguments, 
   the cached value is returned, and the function is _not_ re-evaluated.

   from  http://wiki.python.org/moin/PythonDecoratorLibrary

   seb 20100624: 
   if cacheSize not None:
       hold no more than `cacheSize` results in cache
       this is useful to limit memory usage
       if more calls are made,
       cache is used in FIFO mode, i.e. oldest results are "forgotten" first
   """
   def __init__(self, cacheSize=None):
       ...
       self.memoized_cache = {}
       self.memoized_cache_argsList = [] # seb
       self.memoized_cache_maxLen = cacheSize
   __init__.__doc__ = __doc__
    
   def _setCacheSize(self, cacheSize=None, clearNow=False):
       """
       cacheSize = None --> inifinite memoizing cache
       cacheSize = 0    --> memoizing turned off (for testing !?)
       cacheSize > 0    --> memoizing used for up to cacheSize different arguments (limit used to save memory)

       if clearNow: clear cache completely now

       note: if cacheSize was None, setting to finite size will
             NOT adjust cacheSize, and the cache will always stay
             (by the current size) larger than the given value.
             unless you have clearNow=True, or until you call clearCache later 
       """
       assert cacheSize is None or cacheSize>=0
       self.memoized_cache_maxLen = cacheSize
       if clearNow:
           ...
           if cacheSize is not None:
               while len(self.memoized_cache_argsList) > self.memoized_cache_maxLen:
                   del self.memoized_cache[ self.memoized_cache_argsList[0] ]
                   del self.memoized_cache_argsList[0]
   def _clearCache(self):
       """
       clear entire cache
       """
       self.memoized_cache.clear()
       del self.memoized_cache_argsList[:]
   ...
   def __call__(self, func):
       ...
       def wrapped_f(*args):
           try:
               ...
               if self.memoized_cache_maxLen is not None:
                   while (
                       (self.memoized_cache_maxLen == 0 and len(self.memoized_cache_argsList)) 
                       or
                       ( 0 < self.memoized_cache_maxLen < len(self.memoized_cache_argsList)+1 )
                       ):
                       del self.memoized_cache[ self.memoized_cache_argsList[0] ]
                       del self.memoized_cache_argsList[0]
               value = self.func(*args)
               if self.memoized_cache_maxLen is None or \
                       self.memoized_cache_maxLen > 0:
                   try:
                       self.memoized_cache[args] = value
                   except TypeError:
                           # uncachable -- for instance, passing a list as an argument.
                           # Better to not cache than to blow up entirely.
                           pass
                       ...
                       if self.memoized_cache_maxLen is not None:
                           self.memoized_cache_argsList.append(args)
               return value

       wrapped_f.func_doc = func.__doc__
       wrapped_f.func_name = func.__name__
       wrapped_f.func_defaults = func.func_defaults
       wrapped_f._setMemoizeCacheSize = self._setCacheSize
       wrapped_f._clearMemoizeCache = self._clearCache
       wrapped_f._memoized_cache = self.memoized_cache
       wrapped_f._memoized_cache_argsList = self.memoized_cache_argsList
       return wrapped_f
