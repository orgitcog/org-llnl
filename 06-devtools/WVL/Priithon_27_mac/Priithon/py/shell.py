...
Shift+Enter       Insert new line into multiline command.
...
Tab               tab completing via popup-menu: including
                       var names, func names, string filenames,...
F1                show context help in help-window (if available)
"""
...
import string
_win_slash_trans_table_ascii=string.maketrans('\\', '/')
...
class Shell(editwindow.EditWindow):
    ...
    def __init__(self, parent, id=-1, pos=wx.DefaultPosition,
        ...
        #seb 20110221: removed `__main__.shell` -- use `sys.app.frame.shell` instead
        #seb 20110221 # Add 'shell' to the interpreter's local namespace.
        #seb 20110221 if not self.interp.locals.has_key('shell'): # 20080729: added "if"
        #seb 20110221     self.setLocalShell()
...
Platform: %s""" % \
    ...
    def OnKeyDown(self, event):
        ...
        if not (controlDown or shiftDown) and key == wx.WXK_RETURN:
            if self.CallTipActive():
                self.CallTipCancel()
            self.processLine()
        # Ctrl+Return (Cntrl+Enter) is used to insert a line break.
        elif (controlDown or shiftDown) and key == wx.WXK_RETURN:
            if self.CallTipActive():
                self.CallTipCancel()
        ...
        #seb 20070106: autocompletion 
            ...
            if self.OnKeyDown_tabCompletion():
                # pressing TAB to indent multi-line commands
                event.Skip()
                return

        #seb 20110505: F1 __doc__ into showHtmlInfo frame
        elif not controlDown and key == wx.WXK_F1:
            #if 
            self.OnKeyDown_tabCompletion(useInfoWindow=True)
            #    # pressing TAB to indent multi-line commands
            #    event.Skip()
            return
        ...
        # Home needs to be aware of the prompt.
            ...
            if currpos >= home: # 20051101 '>' changed to '>='
                ...
                event.controlDown = False#20051104 seb
                event.keyCode = wx.WXK_HOME#20051104 seb
                event.Skip()
        #
...
#seb took this out         # Search up the history for the text in front of the cursor.
    ...
    def OnKeyDown_tabCompletion(self, useInfoWindow=False):
        #wx.Bell()
        if self.AutoCompActive():
            self.AutoCompCancel()

        currpos = self.GetCurrentPos() #seb:20110218
        stoppos = self.promptPosEnd
        command = self.GetTextRange(stoppos, currpos)
        #self.autoCompleteShow(command)
        ...
        if len(command) and command[-1] in ('(',):
            self.ReplaceSelection('')
            self.autoCallTipShow(command, alwaysShow=True, useInfoWindow=useInfoWindow)
        else:
            ...
            #20110218: filename completion
            quote_double_pos = command.rfind('"')
            quote_single_pos = command.rfind("'")
            if quote_double_pos>=0 and quote_double_pos>quote_single_pos:
                quote_found = '"'
                quote_found_pos = quote_double_pos
            elif quote_single_pos>=0 and quote_single_pos>quote_double_pos:
                quote_found = "'"
                quote_found_pos = quote_single_pos
            else:
                quote_found = ''
            
            if quote_found:
                if command.count(quote_found) % 2 == 0:
                    quote_found = False # string is closed (left of curpos)
            
            if quote_found:
                end = self.GetTextRange(quote_found_pos+1, self.GetTextLength()).find(quote_found)
                if end<0:
                    end = currpos-stoppos
                else:
                    end += quote_found_pos+1
                
                fn0= command[quote_found_pos+1:end]
                #print >> __main__.shell.stderr,  "DEBUG: FOUND quote", quote_found, fn0
                if '*' not in fn0:
                    fnGlob=fn0+'*'
                else:
                    fnGlob = fn0
                import glob, os.path, sys
                options = glob.glob(fnGlob)
                offset= currpos - stoppos - 1 - quote_found_pos
                if options:
                    options = sorted(map(lambda fn: (fn+"/" # +quote_found 
                                                     if os.path.isdir(fn) 
                                                     else fn+quote_found), 
                                         options ))
                    options = '\n'.join(options)
                    ...
                    if sys.platform =='win32':
                        if isinstance(options, unicode):
                            options=options.translate({ord(u'\\'):u'/'})
                        else:
                            options=options.translate(_win_slash_trans_table_ascii)

                    if len(fn0)>1 and fn0[-1]=='/':
                        options =  fn0+quote_found + "\n" + options
            ...
            #20110819: import <listModules> completion
            elif command.startswith("import "):
                if command == "import ":
                    root = "" # fix behavior; root would be "import" here
                from .. import useful as U
                #import sys
                #sss = sys.app.frame.shell
                #print >> sss.stderr, "DEBUG root:", root
                #print >> sss.stderr, "DEBUG command:", command
                offset = len(root)
                options = ' '.join( [m for m in U.getModules() if m.startswith(root)] )
                self.AutoCompShow(offset, options)


            # 20080908:  argument names completion (works only in simple cases !!)
            elif root=='' and command:
                ...
                if useInfoWindow: # 20110505
                    try:
                        # getCallTip prepends some argspec-text for functions ...
                        name, argspec, tip = self.interp.getCallTip(command)
                        if tip:
                            txt = tip
                        else:  # otherwise we simply try to get '__doc__'
                            obj = eval(root, self.interp.locals)
                            txt = obj.__doc__
                    except:
                        pass
                    else:
                        if txt:
                            from ..all import Y
                            from ..PriDocs import PRE
                            Y.showHtmlInfo(PRE(txt),
                                           title="__doc__ help: %s"%(command,),
                                           size=(700,800))               
                        return
                hasDot = root.rfind('.')
                if hasDot>=0:
    ...
    def push(self, command, newPrompt=True):
        ...
        if 1:
            from .. import PriConfig
            ttt = PriConfig.shellCommandTimer
            if isinstance(ttt, (str, unicode)):
                import time
                ttt = time.__dict__[ttt]
            t0 = ttt()
            self.more = self.interp.push(command)
            if not self.more:
                t0 = ttt()-t0
                sst = self.GetTopLevelParent().SetStatusText
                if t0 < 1:
                    sst("%.3f ms"%(t0*1000,) ,1)
                elif t0 < 10:
                    sst("%.3f s"%(t0,) ,1)
    ...
    def autoCompleteShow(self, command, offset = 0): # seb20070712: moved offset into arguments
        ...
        list = self.interp.getAutoCompleteList(command,
                    ...
                    sortCaseSensitive=self.autoCompleteSortCaseSensitive)
        if list:
            options = ' '.join(list)
            #seb offset = 0
            self.AutoCompShow(offset, options)

    def autoCallTipShow(self, command, alwaysShow=False, useInfoWindow=False): #seb: added alwaysShow for 'tab'-key
        """Display argument spec and docstring in a popup window."""
        if self.CallTipActive():
            self.CallTipCancel()
        (name, argspec, tip) = self.interp.getCallTip(command)
        if tip:
            if useInfoWindow or len(tip)>80*30:
                from ..all import Y
                from ..PriDocs import PRE
                #print len(tip)
                txt=tip
                comm = command[:-1] if command[-1] == '(' else command
                Y.showHtmlInfo(PRE(txt),
                       title="__doc__ help: %s"%(comm,),
                       size=(700,800))
            else:
    ...
    def autosave(self):
        ...
        from ..usefulX import saveSession
        global _first_autosave
        try:
            _first_autosave
        except NameError:
            _first_autosave = True
        else:
            _first_autosave = False
        saveSession(autosave=True, verbose=_first_autosave)
