...
def getAutoCompleteList(command='', locals=None, includeMagic=1, 
                        includeSingle=1, includeDouble=1, sortCaseSensitive=False):
    ...
    try:
        ...
        attributes = getAttributeNames(object, includeMagic, 
                                       includeSingle, includeDouble, sortCaseSensitive)
    return attributes
    
def getAttributeNames(object, includeMagic=1, includeSingle=1,
                      includeDouble=1, sortCaseSensitive=False):
  ...
  try:
    ...
    #seb: support for win32com objects:
    try: attributes += object._prop_map_get_.keys()  # using hasattrib would also only do a try:...except:
    except: pass
    ...
    if sortCaseSensitive:                                         #seb: 20100602
        attributes.sort(lambda x, y: cmp(x, y))                   #seb: 20100602
    else:                                                         #seb: 20100602
...
try:
    ...
    def fnst():
        ...
        for tt in N.ScalarType:
            #20120126 l=[k for k,v in N.__dict__.iteritems() if v == tt]
            l=[k for k,v in N.__dict__.iteritems() if v is tt]
