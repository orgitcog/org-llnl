...
class FileDropTarget(wx.FileDropTarget):
    def __init__(self, parent, pyshell=None):
        ...
        if pyshell is not None:
            ...
            import sys
            self.pyshell = sys.app.frame.shell
    ...
    def OnDropFiles(self, x, y, filenames):
        ...
        if len(filenames) == 1:
            ...
            if os.path.isdir(self.fn_or_fns):
                ...
                m.Append(Menu_appSysPath,   "prepend to sys.path")
            ...
            m.Append(Menu_assignList,  "assign list of filenames to var")
    ...
    def onCd(self, ev):
        ...
        s = "import os; os.chdir(r\"%s\")"%(self.fn_or_fns,)
        Y.shellMessage("###  %s\n"% s)
        self.pyshell.addHistory(s)
    def onAppSysPath(self, ev):
        import sys
        from .all import Y
        #20100311 sys.path.append( self.fn_or_fns )
        #20100311 s = "sys.path.append(r\"%s\")"% (self.fn_or_fns,)
        sys.path.insert(0, self.fn_or_fns )
        s = "import sys; sys.path.insert(0, r\"%s\")"% (self.fn_or_fns,)
        Y.shellMessage("###  %s\n"% s)
        self.pyshell.addHistory(s)
    ...
    def onAssignFN(self, ev):
        v = wx.GetTextFromUser("assign filename to varname:", 'new variable', 'fn')
        if not v:
            return
        import __main__
        try:
    ...
    def onAssignList(self, ev):
        v = wx.GetTextFromUser("assign list to varname:", 'new variable', 'fns')
        if not v:
            return
        import __main__
        try:
    ...
    def onAssignSeq(self, ev):
        from .all import Y
        v = wx.GetTextFromUser("assign image sequence to array varname:", 'new variable', 'imgs')
        if not v:
            return
        import __main__
        try:
    ...
    def onLoadTxt(self, ev):
        v = wx.GetTextFromUser("load 2d table to varname:", 'new variable', 'a')
        if not v:
            return
        import __main__
