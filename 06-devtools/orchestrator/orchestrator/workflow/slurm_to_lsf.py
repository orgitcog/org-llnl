...
class SlurmtoLSFWF(HPCWorkflow, ABC):
    """
    Workflow manager for full execution using a batch file and LSF scheduler

    SlurmtoLSF is a fully featured workflow module for submitting jobs to
    the LSF scheduler using batch files while Orchestrator is running on SLURM.
    It can handle asynchronus job submission but still provides the option
    for blocking (synchronous) behavior. Responsibilities include directory
    creation, job creation, job status checking.
    """
    ...
    def __init__(self, default_template: Optional[str] = None, **kwargs):
        ...
        lsf_machine_name_df = 'lassen'
        self.run_string = kwargs.get('lrun_path', lrun_path_df)
        self.jsrun_string = kwargs.get('jsrun_path', jsrun_path_df)
        self.lsf_profile = kwargs.get('lsf_profile_path', lsf_profile_path_df)
        self.lsf_machine = kwargs.get('lsf_machine_name', lsf_machine_name_df)
    ...
    def update_job_status(self, lsf_ids: list[int]) -> list[str]:
        ...
        and 'completing'. The current status is returned for convinience.
    ...
    def submit_job(
        ...
        interface for the workflows to be used. For the
        :class:`SlurmtoLSFWF` implementation, fully articulated batch
        scripts are generated each job and submitted to the LSF scheduler via
        bsub from another machine. Method inputs define the ``command`` to be
        executed for the job, location for the run, and details about
        the job's resources (``job_details``). ``job_details`` inlcudes
        ``dependencies`` of the job in the form of a list of job_ids,
        ...
        if job_can_run:
            ...
            # note that this command may need to be modified
            # for different compute systems
            submit_command = (f'ssh {self.lsf_machine} "'
                              f'cd {cwd}/{run_path}; '
                              f'source {self.lsf_profile}; '
