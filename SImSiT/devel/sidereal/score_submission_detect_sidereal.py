...
def score_submission(args):
    ...
    if path.exists('score_history_detect_sidereal.txt'):
        scores.to_csv('score_history_detect_sidereal.txt', mode='a', index=False, header=False)
    else:
        scores.to_csv("score_history_detect_sidereal.txt", index=False)
