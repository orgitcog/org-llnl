 ...
 *     std::string s = axom::fmt::format("{}"_cf, 42);
 */
...
template <typename Char, typename V, int N> struct spec_field {
  ...
  template <typename OutputIt, typename... T>
  constexpr AXOM_FMT_INLINE auto format(OutputIt out, const T&... args) const
      -> OutputIt {
    const auto& vargs =
...
AXOM_FMT_INLINE AXOM_FMT_CONSTEXPR_STRING auto format(const CompiledFormat& cf,
                                            const T&... args)
...
template <typename OutputIt, typename CompiledFormat, typename... T,
          AXOM_FMT_ENABLE_IF(detail::is_compiled_format<CompiledFormat>::value)>
constexpr AXOM_FMT_INLINE auto format_to(OutputIt out, const CompiledFormat& cf,
...
template <typename S, typename... T,
          AXOM_FMT_ENABLE_IF(is_compiled_string<S>::value)>
AXOM_FMT_INLINE AXOM_FMT_CONSTEXPR_STRING auto format(const S&, T&&... args)
  ...
  if constexpr (std::is_same<remove_cvref_t<decltype(compiled)>,
    ...
    return axom::fmt::format(compiled, std::forward<T>(args)...);
  }
}

template <typename OutputIt, typename S, typename... T,
          AXOM_FMT_ENABLE_IF(is_compiled_string<S>::value)>
AXOM_FMT_CONSTEXPR auto format_to(OutputIt out, const S&, T&&... args) -> OutputIt {
  constexpr auto compiled = detail::compile<T...>(S());
  if constexpr (std::is_same<remove_cvref_t<decltype(compiled)>,
                             detail::unknown_format>()) {
    ...
    return axom::fmt::format_to(out, compiled, std::forward<T>(args)...);
  }
}
#endif
...
auto format_to_n(OutputIt out, size_t n, const S& fmt, T&&... args)
  ...
  axom::fmt::format_to(std::back_inserter(buf), fmt, std::forward<T>(args)...);
  return {buf.out(), buf.count()};
}

template <typename S, typename... T,
          AXOM_FMT_ENABLE_IF(is_compiled_string<S>::value)>
AXOM_FMT_CONSTEXPR20 auto formatted_size(const S& fmt, T&&... args) -> size_t {
  auto buf = detail::counting_buffer<>();
  axom::fmt::format_to(appender(buf), fmt, std::forward<T>(args)...);
  return buf.count();
}
...
template <size_t N> class static_format_result {
 ...
 public:
  template <typename S, typename... T,
            AXOM_FMT_ENABLE_IF(is_compiled_string<S>::value)>
  explicit AXOM_FMT_CONSTEXPR static_format_result(const S& fmt, T&&... args) {
    *axom::fmt::format_to(data, fmt, std::forward<T>(args)...) = '\0';
  }

  auto str() const -> axom::fmt::string_view { return {data, N - 1}; }
 ...
 * a `axom::fmt::string_view` via `str()`.
 *
 * **Example**:
 *
 *     // Produces the static string "42" at compile time.
 *     static constexpr auto result = AXOM_FMT_STATIC_FORMAT("{}", 42);
 *     const char* s = result.c_str();
 */
#define AXOM_FMT_STATIC_FORMAT(fmt_str, ...)                            \
  axom::fmt::static_format_result<                                       \
      axom::fmt::formatted_size(AXOM_FMT_COMPILE(fmt_str), __VA_ARGS__) + 1>( \
      AXOM_FMT_COMPILE(fmt_str), __VA_ARGS__)
