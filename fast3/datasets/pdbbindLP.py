...
class PDBBind(Dataset):
    ...
    def __init__(
        ...
        self.max_lpatoms = configs["max_lpatoms"] if "max_lpatoms" in configs else 1200
        self.max_patoms = configs["max_patoms"] if "max_patoms" in configs else 1200
        self.max_latoms = configs["max_latoms"] if "max_latoms" in configs else 250
        self.cutoff = configs["cutoff"] if "cutoff" in configs else 5
...
class PDBBind2016(PDBBind):
    ...
    def __getitem__(self, item):
        ...
        if self.use_docking:
            ...
            data = self.df[pdbid][self.feature_type][self.preprocessing_type]["pdbbind"]["data"]
            vdw_radii = (
                self.df[pdbid][self.feature_type][self.preprocessing_type]["pdbbind"]
                .attrs["van_der_waals"]
                .reshape(-1, 1)
            )
...
class PDBBind2020(PDBBind):
    ...
    def __getitem__(self, item):
        ...
        if "graph" in self.input_type:
            ...
            protein_index = (x[:, 14]!=1).nonzero().view(-1)
            

            dists = dists[protein_index, :]
            dists = dists[:, ligand_index]
            edge_index, edge_attr = dense_to_sparse(torch.from_numpy(dists))
        ...
        # Only consider ligand-protein x and edges
        inputs = {
            "data": _data
        }
