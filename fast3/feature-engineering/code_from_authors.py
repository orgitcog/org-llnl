## this is code from the authors that they used to 
## preprocess the clusters. We're using it for their kmer/PSC features
...
def get_ecfp_encoding(smiles, radius=2, nBits=1024):
    ...
    for smile in smiles:
        ...
        smiles_lst.append(smile)
    ecfp_lst = np.array(ecfp_lst)
    return smiles_lst, ecfp_lst
...
def get_3mer_encoding(aa_sequences):
    kmers = get_3mers()
    return aa_sequences, get_kmers_encoding(aa_sequences, kmers)
...
def unseen_cluster_pair_test_split(df, test_size=0.4):
    ...
    target_train_df = target_df.sample(frac=0.8, random_state=45)
...
if __name__ == "__main__":
    df = pd.read_csv("datasets/biosnap/random/full.csv")
