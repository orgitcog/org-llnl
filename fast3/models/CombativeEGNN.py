...
class CEGNN(BaseModel):
    def __init__(
        ...
        for i in range(0, n_p_layers):
            self.add_module("gcl_p_%d" % i, E_GCL(input_nf=hidden_channels,output_nf=hidden_channels, hidden_nf=hidden_channels))
            self.add_module("tanh_p_%d" % i, nn.Tanh())
            self.add_module("bn_p_%d" % i, nn.BatchNorm1d(hidden_channels))
        
        for i in range(0, n_l_layers):
            self.add_module("gcl_l_%d" % i, E_GCL(input_nf=hidden_channels,output_nf=hidden_channels, hidden_nf=hidden_channels))
            self.add_module("tanh_l_%d" % i, nn.Tanh())
            self.add_module("bn_l_%d" % i, nn.BatchNorm1d(hidden_channels))
        
        for i in range(0, n_lp_layers):
            self.add_module("gcl_lp_%d" % i, E_GCL(input_nf=hidden_channels,output_nf=hidden_channels, hidden_nf=hidden_channels))
            self.add_module("tanh_lp_%d" % i, nn.Tanh())
            self.add_module("bn_lp_%d" % i, nn.BatchNorm1d(hidden_channels))

        self.LigandTransEnc = nn.TransformerEncoderLayer(
        ...
        self.ProteinTransEnc = nn.TransformerEncoderLayer(
            d_model=hidden_channels, 
            nhead=2, 
            dim_feedforward=dim_feedforward,
            dropout=dropout,
        ...
        self.transDec = nn.TransformerDecoderLayer(
            d_model=hidden_channels,
            nhead=2,
            dim_feedforward=dim_feedforward,
            dropout=dropout,
    ...
    def _forward(self, data):
        ...
        for i in range(0, self.n_p_layers):
            if i ==0:
                ...
                coords_hat_p = self._modules["tanh_p_%d" %i](hbar_p)
                coords_hat_p = self._modules["bn_p_%d" %i](hbar_p)
        ...
        for i in range(0, self.n_l_layers):
            if i ==0:
                ...
                coords_hat_l = self._modules["tanh_l_%d" %i](hbar_l)
                coords_hat_l = self._modules["bn_l_%d" %i](hbar_l)
            else:
                hbar_l, coords_hat_l, _ = self._modules["gcl_l_%d" % i](
                    h=hbar_l, 
                    edge_index=l_features["edge_index"],
        ...
        for i in range(0, self.n_lp_layers):
            if i == 0:
                ...
                coords_hat_lp = self._modules["tanh_lp_%d" %i](hbar_lp)
                coords_hat_lp = self._modules["bn_lp_%d" %i](hbar_lp)
            else:
                hbar_lp, coords_hat_lp, _ = self._modules["gcl_lp_%d" % i](
                    h=hbar_lp, 
                    edge_index=lp_features["edge_index"],
        ...
        # Get the features from protein-protein and feed it to a protein transformer
        p_embedding = self.ProteinTransEnc(coords_hat_p)

        # Get the features from protein-protein and feed it to a ligand transformer
        l_embedding = self.LigandTransEnc(coords_hat_l)
        
        # Pool based on batch
        p_embedding = self.pool(p_embedding, data.batch)
        l_embedding = self.pool(l_embedding, data.batch)
        lp_embedding = self.pool(coords_hat_lp, data.batch)

        # The memory is the embeddings of the ligand-ligand and protein-protein features, concatenated and reduced in dimensions
        mem_input = torch.cat([p_embedding, l_embedding], axis=1)
        mem_input = self.mlp_mix(mem_input)

        # The target is the ligand-protein binding
        tgt_input = self.mlp_lp(lp_embedding)

        # Feed to a decoder and get the [batch_size, 1] affinity vector 
        # Using an mlp
        out = self.transDec(tgt= tgt_input, memory=mem_input)
