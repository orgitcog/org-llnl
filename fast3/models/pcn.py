...
class PCN(BaseModel):
    ...
    def __init__(
        ...
        in_channels += 3
        super(PCN, self).__init__(in_channels, out_channels, loss_fn)
        ...
        for filter_size in self.filter_sizes:
            w = torch.tensor(truncnorm.rvs(-1, 1, size=[self.num_filters,1,filter_size,self.in_channels])).float()
    ...
    def _forward(self, data, get_feature=False):
        ...
        for filter_size, w, b in zip(self.filter_sizes, self.encoder_param_w, self.encoder_param_b):
            ...
            h = torch.relu(c)
            pooled = func.max_pool2d(h, kernel_size=[self.max_atoms-filter_size+1,1], stride=1, padding=0)
            if self.verbose != 0:
                print(pooled.shape)
