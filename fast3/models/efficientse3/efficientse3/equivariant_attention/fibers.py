...
class Fiber(object):
    ...
    @staticmethod
    def combine_fibers(val1, struc1, val2, struc2, return_fiber_obj=False):
        """
        combine two fibers
...
def fiber2head(F, h, structure, squeeze=False):
    if squeeze:
        fibers = [F[f'{i}'].reshape(*F[f'{i}'].shape[:-2], h, -1) for i in structure.degrees]
        fibers = torch.cat(fibers, -1)
    else:
        fibers = [F[f'{i}'].reshape(*F[f'{i}'].shape[:-2], h, -1, 1) for i in structure.degrees]
        fibers = torch.cat(fibers, -2)
    return fibers
