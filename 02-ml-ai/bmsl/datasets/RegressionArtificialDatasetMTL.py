...
# @Last Modified time: 2019-07-24 09:22:27
import numpy as np
import pandas as pd
from design import DatasetMTL


class RegressionArtificialDatasetMTL(DatasetMTL):
    """
...
class RegressionArtificialDatasetMTL_SingleGroup(DatasetMTL):
    """
        Implement an MTL Artificial Regression dataset generator.
        The generated data is mainly to test Bayesian MTL recovery capabilities.
    """
    ...
    def prepare_data(self):
        ...
        for t in range(self.nb_tasks):
            ...
            if (t < 10):  # int(self.nb_tasks / 2)):  # first half of the tasks
                self.data['true_w']['w_task_{}'.format(t)] = w_group_1 + self.wg1_eps * np.random.randn(self.dimension, 1)
            else:  # second half of the tasks
                self.data['true_w']['w_task_{}'.format(t)] = self.wg2_eps * np.random.randn(self.dimension, 1)
