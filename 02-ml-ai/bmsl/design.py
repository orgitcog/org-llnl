...
# @Date:   2019-06-26 15:26:02
# @Last Modified by:   Andre Goncalves
# @Last Modified time: 2019-07-24 12:50:34
import os
import types
import pickle
import shutil
...
THE_HIGHER_THE_BETTER = ('area_under_curve', 'avg_precision', 
                         'weighted_accuracy', 'accuracy', 
                         'accuracy_per_class', 'c_index',
                         'c_index_ours')
THE_LOWER_THE_BETTER = ('rmse', 'nmse', 'rmse_survival',
                        'mse_survival', 'brier_score',
                        'mae_survival')
...
class ModelTraining(object):
    ...
    def execute(self, dataset, methods, metrics, nb_runs=1, report_only=False):
    ...
    def generate_report(self):
        ...
        tasks_name_mapping = dict(zip(tasks_orig, tasks_new))
        task_names = pd.DataFrame(tasks_name_mapping.items(), columns=['Original', 'New name'])

        # save results table into latex format
        txt_filename = os.path.join(config.path_to_output,
                                    self.name,
                                    '{}_table.tex'.format(self.name))
        dfa = df[['Run', 'Method', 'Task', 'Metric', 'Value']].copy()
        dfb = dfa.groupby(['Method', 'Task', 'Metric']).agg(['mean', 'std'])
        print(dfb)

        with open(txt_filename, 'w') as fh:
            fh.write(df.to_latex())

        task_names.to_csv(os.path.join(config.path_to_output,
                          self.name, 'tasks_name_mapping.csv'), index=False)
        ...
        self.__performance_plots_per_task(df, op_dict, pdf, tasks_orig, tasks_name_mapping)
        # self.__plot_calibration_curves(df, op_dict, pdf)
    ...
    def __performance_plots_per_task(self, df, op_dict, pdf, tasks_orig, tasks_map):
        """ Compute performance metric of all test samples regardless the task.
        Data from all tasks are pooled and then the performance is computed. """
    ...
    def __plot_calibration_curves(self, df, op_dict, pdf):
        ...
        for j, method in enumerate(methods):
            ...
            fraction_of_positives, mean_predicted_value = \
                calibration_curve(accum_observed, accum_pred, n_bins=10)

            plt.plot(mean_predicted_value, fraction_of_positives, "s-", label="%s" % (method, ))
