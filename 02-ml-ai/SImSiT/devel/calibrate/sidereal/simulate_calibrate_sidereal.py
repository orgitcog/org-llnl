...
def sky_brightness():
    ...
    # zeropoint from David Kirkby notes in photons per second per pixel
    # in i band
    s0 = area*1.249
    ...
    sbar = area_ratio * s0 * 10**(-0.4*(B-24))
    print()
    print("Base sky brightness")
    print(f"{sbar:.2f} photons per square arcsecond per second")
...
def init_image(config, wcs, sky_phot):
    """Initialize an image.
    ...
    # Sky background
    wcs.makeSkyImage(image, sky_phot*config['exptime'])
    print()
...
def get_streaks(config, rng, image):
    ...
    streak_length = streak_rate * config['exptime'] / scale
...
def make_image(config, rng):
    # Fiducial sky background for Cerro Pachon
    sky_phot, sky_mag = sky_brightness()
    # Add some variability to the background.  Always assume ssa site is worse
    # than Cerro Pachon.
    ud = galsim.UniformDeviate(rng)
    dmag = 0.3 + 0.5*ud()
    print(f"modifying sky brightness by {dmag:.2f} magnitudes")
    sky_mag -= dmag
    sky_phot *= 10**(0.4*dmag)
    print("after modification:")
    print(f"{sky_phot:.2f} photons per square arcsecond per second")
    print(f"{sky_mag:.2f} magnitudes per square arcsecond")

    time, observer = generate_time_and_observer(config, rng)
    wcs = generate_wcs(config, rng, time, observer)
    image = init_image(config, wcs, sky_phot)
    ...
    streaks = get_streaks(config, rng, image)
    streaks['nphot'] = sky_phot * 10**(-0.4 * (streaks['mag']-sky_mag))
    streaks['nphot'] *= config['exptime']

    draw_streaks(image, streaks, psf)

    stars = get_stars(config, image, time, observer)
    stars['nphot'] = sky_phot * 10**(-0.4 * (stars['i_mag']-sky_mag))
    stars['nphot'] *= config['exptime']
    ...
    return image, stars, streaks, time, observer


def simulate_calibrate_sidereal(args):
    ...
    for i_obs in range(config['n_obs']):
        ...
        print()
        img, stars, streaks, time, observer = make_image(config, rng)
        stars.meta = {}
        ...
        if i_obs == (config['n_demo']-1):
            fn = "public/sample_submission_5_calibrate_sidereal.yaml"
            with open(fn, "w") as f:
        ...
        hduTrueWCS.header['DATE-END']=(time+config['exptime']*u.s).tai.isot
        hduTrueWCS.header['TELAPSE']=config['exptime']
        ...
        hdu.header['DATE-END']=(time+config['exptime']*u.s).tai.isot
        hdu.header['TELAPSE']=config['exptime']
    ...
    truth_demo_hdul.writeto("public/truth_5_calibrate_sidereal.fits", overwrite=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config",
        type=str,
        default="simulate_calibrate_sidereal_config.yaml"
    )
    args = parser.parse_args()

    simulate_calibrate_sidereal(args)
