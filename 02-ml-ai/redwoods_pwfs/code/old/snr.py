...
def image(mag, t_int, amp, wfe_phase, phn=False, readn= False):
	...
	tr_atm = 0.30 # transmission through atmosphere
	th = 0.20 # instrument throughput
...
def signal_pl(mag, t_int, amp, cm):
	...
	'''
	pl_wfe = image(mag, t_int, amp, wfe_phase='pl')#+ image(mag, t_int, amp, wfe_phase='zernike')#- image(mag, t_int, amp, wfe_phase='none')
...
def noise_coeff(mag, t_int, amp, cm):
	...
	'''
	#photon_noise = image(mag, t_int, amp, wfe_phase='pl', phn=True, readn=True) - image(mag, t_int, amp, wfe_phase='pl', phn=False, readn=False) #(image(mag, t_int, amp, wfe_phase='fourier', phn=True, readn=False) + image(mag, t_int, amp, wfe_phase='zernike', phn=True, readn=False))/2 # added read noise twice, one for each mode
	...
	noise =  image(mag, t_int, amp, wfe_phase='pl', phn=True, readn=True)
...
def t_snr(t, mag, i_amp, n):
	...
	for m in range(len(mag)):
		...
		nz_1 = np.zeros(n)
		cm = CM(mag[m], t, i_amp)
		#only need to compute signal term once since it doesnt change
...
def snr_mag(i_amp, n=25):
	...
	plt.yscale('log')
	plt.tight_layout()
	plt.savefig('./plots/snr_mag.png', transparent=False, dpi=400)
	plt.show()
