...
def first_pass(m_object, t_int):
    '''
    Copy of first pass with M: 0, t_int: 1ms to 10 ms
    '''
    read_noise_level=1
    wav0=0.95e-6 # NOT SURE WHAT WAVELENGTH TO USE, DEFAULT WAS 0.95 WHICH IS AT THE BOTTOM OF BANDPASS
    imagepix=128
    pupilpix=29
    ...
    tr_atm,th,qe=0.9,0.2,0.75 #assume transmission through the atmosphere, instrument throughput, quantum efficiency of CCD
    ...
    def propagate(im_tar_phase, aperture, phn = False):
        pupil_wavefront_dm_unnorm=aperture*np.exp(1j*im_tar_phase)
    ...
    plots = np.array([[im, imn, diff], 
                      [im, diff_phn, im_phn], 
                      [im, both, diff_both]])
    
    headings = np.array([['no noise', 'read noise', 'real intensity'],
                         ['no noise', 'light noise', 'real intensity'],
                         ['no noise', 'both noise', 'real intensity']])
    print(m_object, t_int)
    return plots, headings, im, both

def plot(m_object, t_int):
    plots, headings, _, _ = first_pass(m_object, t_int)
    ...
    plt.tight_layout()
    plt.savefig('./plots/first_pass_testing.png', dpi = 400)
    plt.close()
    plt.pause(1.75)
    plt.close()

def main():
    magnitude = np.linspace(1, 15, 15)
    exposure = 1e-3 * np.ones(len(magnitude))

    for mag, exp in zip(magnitude, exposure):   
        plot(mag, exp)
