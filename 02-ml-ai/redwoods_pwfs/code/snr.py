...
def image(mag, t_int, amp, wfe_phase, antialiased=True,  phn=False, readn= False):
...
def im_prop(mag, t_int, i_amp):
	...
	zf_wfe = image(mag, t_int, i_amp, wfe_phase='pl') #- image(mag, t_int, i_amp, wfe_phase='none')
	noise = image(mag, t_int, i_amp, wfe_phase='pl', phn=True, readn=True)  #- image(mag, t_int, i_amp, 'none', phn=False, readn=False)
	phn = image(mag, t_int, i_amp, wfe_phase='pl', phn=True, readn=False) - image(mag, t_int, i_amp, 'none', phn=False, readn=False)

	# save plots/headers
	plots = [[no_wfe, zf_wfe], [noise, phn]]
	header = [['No WFE', 'WFE'], ['Photon + Read Noise', 'Photon Noise']]
	row, col = 2, 2
...
def t_snr_single(mag, t_int, i_amp, n):
	sf_1 = np.zeros(n)
	nf_1 = np.zeros(n)
	sz_1 = np.zeros(n)
	nz_1 = np.zeros(n)
	cm = CM(mag, t_int, i_amp)
	for i in range(n):
		coeff_sig = signal_pl(mag, t_int, i_amp, cm)
		coeff_noise = noise_coeff(mag, t_int, i_amp, cm)
	...
	f_snr = (np.mean(sf_1)) / (np.std(nf_1, ddof=1))
	z_snr = (np.mean(sz_1) / (np.std(nz_1, ddof=1)))
	return f_snr, z_snr
...
def snr_mag(i_amp, n=500):
	...
	for t in range(len(t_list)):
		args = [(m, t_list[t], i_amp, n) for m in mag]
		with multiprocessing.Pool(processes=num_cores) as pool:
			results = pool.starmap(t_snr_single, args)
		f_snr, z_snr = zip(*results)
	...
	# saving to file
	with open('./python/data/snr.txt', 'a') as f:
		np.savetxt(f, [data[0], data[1], data[2], data[3], data[4]], header=f'd_tel = 10m, read_noise = 1, n={n}\n	rows are: f_snr_1ms, z_snr_1ms, f_snr_10ms, z_snr_10ms')

	# plotting
	plt.figure()
	plt.plot(data[0], np.abs(data[1]), label='Fourier mode (1ms)', marker='x' , c=(14/255, 96/255, 39/255))
	plt.plot(data[0], np.abs(data[2]), label='Zernike mode (1ms)', marker='x' , c=(0/255, 83/255, 154/255))
	plt.plot(data[0], np.abs(data[3]), label='Fourier mode (10ms)', marker='o' ,  c=(14/255, 96/255, 39/255))
	plt.plot(data[0], np.abs(data[4]), label='Zernike mode (10ms)', marker='o' , c=(0/255, 83/255, 154/255))
