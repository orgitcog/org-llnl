# For LTAU-FF, this file should be replace nequip/train/trainer.py
...
class Trainer:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.learn_events_log = {
            'energy': output.open_logfile(
                f"energy_learn_logger.csv", propagate=False
            ),
            'forces': output.open_logfile(
                f"forces_learn_logger.csv", propagate=False
            ),
        }

        self.log_epoch_errors = log_epoch_errors
    ...
    def init_metrics(self):
        ...
        if self.log_epoch_errors:
            total_natoms = sum([data['pos'].shape[0] for data in self.dataset_train])

            self.energy_errors = torch.zeros(self.n_train, device=self.device)
            self.forces_errors = torch.zeros(total_natoms, device=self.device)
    ...
    def batch_step(self, data, validation=False):
        ...
        with torch.no_grad():
            ...
            if self.log_epoch_errors and not validation:
                # for indexing into dataset-wide arrays
                e_indices = data['supercell-idx'].type(torch.int64).clone()
                f_indices = data['atom-idx'].type(torch.int64).clone()

                _emax_ind = torch.max(e_indices)
                _fmax_ind = torch.max(f_indices)

                if _emax_ind >= self.energy_errors.shape[0]:
                    raise RuntimeError('Attempting to access index {} for error logging, but array size is {}. Check that supercell-idx values are correct.'.format(_emax_ind, self.energy_errors.shape))

                if _fmax_ind >= self.forces_errors.shape[0]:
                    raise RuntimeError('Attempting to access index {} for error logging, but array size is {}. Check that atom-idx values are correct.'.format(_fmax_ind, self.forces_errors.shape))

                # Re-compute absolute error because it wasn't saved anywhere
                energy_err = torch.abs(
                    data[AtomicDataDict.TOTAL_ENERGY_KEY]
                    - out[AtomicDataDict.TOTAL_ENERGY_KEY]
                )

                natoms =  torch.bincount(data[AtomicDataDict.BATCH_KEY])

                energy_err /= natoms[:, None]

                forces_err = torch.linalg.norm(
                ...
                # Log errors
                if len(energy_err.shape) > 1:  # for batch_size > 1
                    energy_err = energy_err[:, 0]
                    assert energy_err.shape[0] == len(e_indices), f'Energy error shape is {energy_err.shape}, but should have length of {len(e_indices)}'

                self.energy_errors[e_indices] = energy_err
                self.forces_errors[f_indices] = forces_err
    ...
    def end_of_epoch_log(self):
        ...
        if self.log_epoch_errors:
            ...
            self.energy_errors = torch.nan_to_num(self.energy_errors, nan=-1, posinf=-1, neginf=-1)
            self.forces_errors = torch.nan_to_num(self.forces_errors, nan=-1, posinf=-1, neginf=-1)

            # log raw errors
            np.save(
                os.path.join(
                    self.output.workdir,
                    f'epoch_{self.iepoch}_energy_errors.npy'
                ),
                self.energy_errors,
            )
            ...
            np.save(
                os.path.join(
                    self.output.workdir,
                    f'epoch_{self.iepoch}_forces_errors.npy'
                ),
                self.forces_errors,
            )

            self.energy_errors[:] = 0
            self.forces_errors[:] = 0
