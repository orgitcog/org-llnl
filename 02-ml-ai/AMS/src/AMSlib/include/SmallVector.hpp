...
// AMS: modified from llvm::SmallVector.
// removed LLVM_NODISCARD, LLVM_LIKELY
...
protected:
  ...
  void *replaceAllocation(void *NewElts,
                          size_t TSize,
                          size_t NewCapacity,
                          size_t VSize = 0);
...
template <class T>
using SmallVectorSizeType = std::
    conditional_t<sizeof(T) < 4 && sizeof(void *) >= 8, uint64_t, uint32_t>;
...
SmallVectorImpl<T> &SmallVectorImpl<T>::operator=(const SmallVectorImpl<T> &RHS)
{
  ...
  this->uninitialized_copy(RHS.begin() + CurSize,
                           RHS.end(),
                           this->begin() + CurSize);
...
SmallVectorImpl<T> &SmallVectorImpl<T>::operator=(SmallVectorImpl<T> &&RHS)
{
  ...
  this->uninitialized_move(RHS.begin() + CurSize,
                           RHS.end(),
                           this->begin() + CurSize);
...
struct CalculateSmallVectorDefaultInlinedElements {
  ...
  static_assert(sizeof(T) <= 256,
                "You are trying to use a default number of inlined elements "
                "for "
                "`SmallVector<T>` but `sizeof(T)` is really big! Please use an "
                "explicit number of inlined elements with `SmallVector<T, N>` "
                "to make "
...
extern template class ams::SmallVectorBase<uint32_t>;
#if SIZE_MAX > UINT32_MAX
extern template class ams::SmallVectorBase<uint64_t>;
#endif
...
template <typename T>
inline void swap(ams::SmallVectorImpl<T> &LHS, ams::SmallVectorImpl<T> &RHS)
{
  LHS.swap(RHS);
}
...
/// Implement std::swap in terms of SmallVector swap.
template <typename T, unsigned N>
inline void swap(ams::SmallVector<T, N> &LHS, ams::SmallVector<T, N> &RHS)
{
  LHS.swap(RHS);
}
