...
outfile = 'gmd_protese2_docking_v4.h5'
...
# for path in Path('/usr/workspace/atom/PostEra/docking_data').rglob('*dock_proc1.hdf5'):
...
for dp in dock_procs:
    ...
    with DockHDF_pdbqt(dockingfile, 'r') as h5_file_object:
        for idx, dock_id in enumerate(dock_ids):
                ...
                poses = h5_file_object.get_pdbqt_pybel('coval', dock_id, compute_partial_charge=True)
