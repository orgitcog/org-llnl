...
# from equivariant_attention.utils.utils_logging import log_gradient_norm
...
class RadialFunc(nn.Module):
    ...
    def __init__(self, num_freq, in_dim, out_dim, edge_dim: int=0, save_params=False):
        """NN parameterized radial profile function.
        ...
        if save_params:
            self.net = nn.Sequential(nn.LeakyReLU(), BN(self.edge_dim+1), nn.Linear(self.edge_dim+1, self.num_freq*in_dim*out_dim))
        else:
...
class PairwiseConv(nn.Module):
    ...
    def __init__(self, degree_in: int, nc_in: int, degree_out: int,
                 nc_out: int, edge_dim: int=0, save_params=False, share_filter=False):
        ...
        self.nc_in = nc_in
        self.nc_out = nc_out # not used, 1x1 conv takes care of nc_in->nc_out later
        self.share_filter = share_filter

        # Functions of the degree
        self.num_freq = 2*min(degree_in, degree_out) + 1
        self.d_out = 2*degree_out + 1
        self.d_in = 2*degree_in + 1
        self.edge_dim = edge_dim

        # Radial profile function
        # only same channels interact so setting 2nd and 3rd args to 1
        if self.share_filter:
            self.rp = RadialFunc(self.num_freq, 1, 1, self.edge_dim, save_params)
        else:
            self.rp = RadialFunc(self.num_freq, in_dim=1, out_dim=nc_in, edge_dim=self.edge_dim, save_params=save_params) # num channel is conserved and coeff depends on output channel

    @profile
    def forward(self, feat, basis):
        # Get radial weights
        R = self.rp(feat)
        kernel = torch.sum(R * basis[f'{self.degree_in},{self.degree_out}'], -1) # (E, c_in, d_out, 1, d_in)
        if self.share_filter:
            return kernel.view(kernel.shape[0], 1, self.d_out, self.d_in).repeat(1, self.nc_in, 1, 1) #(E, c_in, d_out, d_in)
        else:
            return kernel.view(kernel.shape[0], self.nc_in, self.d_out, self.d_in) #(E, c_in, d_out, d_in)
...
class GConvSE3Partial(nn.Module):
    ...
    def __init__(self, f_in, f_out, edge_dim: int=0, x_ij=None, save_params=False, share_filter=False):
        ...
        # Node -> edge weights
        self.kernel_unary = nn.ModuleDict()
        self.W_1x1 = nn.ParameterDict()
        for (mi, di) in self.f_in.structure:
            for (mo, do) in self.f_out.structure:
                # multiplicities will be ignored
                self.kernel_unary[f'({di},{do})'] = PairwiseConv(di, mi, do, mo, edge_dim=edge_dim, save_params=save_params, share_filter=share_filter)
                # mix channels with a 1x1 conv
                self.W_1x1[f'({di},{do})'] = nn.Parameter(torch.randn(mo, mi) / np.sqrt(mi))
    def __repr__(self):
        ...
        def fnc(edges):
            ...
            for m_in, d_in in self.f_in.structure:
                ...
                if self.x_ij == 'cat' and d_in == 1:
                    ...
                    src = edges.src[f'{d_in}'].view(-1, m_in, (2*d_in+1), 1) #(E, c_in, dim_in)
                src = src.view(-1, m_in, (2*d_in+1), 1)
                edge = edges.data[f'({d_in},{d_out})']
                # msg = msg + torch.matmul(edge, src)
                new_msg = torch.einsum('ecij, ecjx->ecix', edge, src).squeeze(-1) # (E, c_in, dim_in), not sure why they had that last dim
                new_msg = torch.einsum('cd, edi->eci', self.W_1x1[f'({d_in},{d_out})'], new_msg) # (E, c_out, dim_out)
                msg = msg + new_msg #torch.einsum('eij, ecj->eci', edge, src) # (E, dim_out, dim_in) (E, c_in, dim_in)
            # msg = msg.view(msg.shape[0], m_in, 2*d_out+1) # (E, c_in, dim_out)
            # msg = torch.einsum('cd, edi->eci', self.W_1x1[f'({d_in},{d_out})'], msg) # (E, c_out, dim_out)
            return {f'out{d_out}': msg.view(msg.shape[0], -1, 2*d_out+1)} # (E, c_out, dim_out)
        return fnc
...
class GMABSE3(nn.Module):
    ...
    def __init__(self, f_value: Fiber, f_key: Fiber, n_heads: int, dropout=0., extensive=False):
    ...
    def forward(self, v, k: Dict=None, q: Dict=None, G=None, **kwargs):
        ...
        with G.local_scope():
            ...
            if not (q == None or k == None):
                ...
                # print(G.edata['k'].shape, G.ndata['q'].shape)
                ...
                e = torch.zeros(G.edata['w'].shape[0], self.n_heads).to(G.device) # uniform attention
            if 'pair_bias' in G.edata:
                e = e + G.edata['pair_bias'].view([G.edata['w'].shape[0], self.n_heads])
            if 'distance_bias' in G.edata:
                assert G.edata['distance_bias'].repeat([1, self.n_heads]).shape == e.shape, 'distance bias has wrong shape'
                e = e + G.edata['distance_bias'].repeat([1, self.n_heads])
            if self.extensive:
                G.edata['a'] = e.sigmoid()
            else:
                G.edata['a'] = edge_softmax(G, e)
            G.edata['a'] = F.dropout(G.edata['a'], p=self.dropout, training=self.training)
            ...
            for m, d in self.f_value.structure:
                ...
                # print(G.ndata[f'out{d}'].shape, '->', output[f'{d}'].shape)
...
class GSE3Res(nn.Module):
    ...
    def __init__(self, f_in: Fiber, f_out: Fiber, edge_dim: int=0, div: float=4,
                 n_heads: int=1, learnable_skip=True, skip='cat', selfint='1x1', x_ij=None, dropout=0., pair_bias=True, uniform_attention=False, save_params=False, share_filter=False, extensive=False):
        super().__init__()
        self.f_in = f_in
        self.f_out = f_out
        self.div = div
        ...
        # Projections
        self.GMAB['v'] = GConvSE3Partial(f_in, self.f_mid_out, edge_dim=edge_dim, x_ij=x_ij, save_params=save_params, share_filter=share_filter)
        if not self.uniform_attention:
            self.GMAB['k'] = GConvSE3Partial(f_in, self.f_mid_in, edge_dim=edge_dim, x_ij=x_ij, save_params=save_params, share_filter=share_filter)
            self.GMAB['q'] = G1x1SE3(f_in, self.f_mid_in)

        # Attention
        self.GMAB['attn'] = GMABSE3(f_value=self.f_mid_out, f_key=self.f_mid_in, n_heads=n_heads, dropout=dropout, extensive=self.extensive)
        if self.pair_bias:
            self.pair_bias_linear = nn.Linear(self.edge_dim, self.n_heads, bias=False)
    ...
    def forward(self, features, G, **kwargs):
        ...
        # Attention
        if self.edge_dim > 0 and self.pair_bias:
            G.edata['pair_bias'] = self.pair_bias_linear(G.edata['w'].view(-1, self.edge_dim))
