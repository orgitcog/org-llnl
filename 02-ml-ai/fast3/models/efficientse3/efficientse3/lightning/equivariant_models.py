...
class EfficientSE3TransformerLBASystem(BaseModel):
    def __init__(self, in_dim, out_dim, edge_dim, optimizer=None, schedule_lr=False,
        ...
        self.atom3d = atom3d
        super().__init__(in_dim, out_dim, optimizer, schedule_lr)
        
        self.mse = nn.MSELoss()
        self.pair_distance = torch.nn.PairwiseDistance(p=2)
        if self.use_mse:
            self.print('Using MSE')  
    ...
    def pred_affinity(self, G):
        ...
        edge_feat = edge_feat.to(self.device) # this should be handled by PyTorch lightning
    ...
    def loss(self, aff_pred, aff_target):
        ...
        # print(f"loss: aff_target.shape={aff_target.shape}, aff_pred.shape={aff_pred.shape}")

        # import ipdb 
        # ipdb.set_trace()


        aff_target = aff_target.to(aff_pred.device)
    ...
    def ptg_to_dgl(self, data, distance_cutoff=5.0):
        ...
        # distances = data.pdists + 1e-4
        distances = data.edge_attr + 1e-4
        ...
        #DEREK EDIT
        # self.edge_attr = data.edge_attr.view(-1,1)
        ...
        # import ipdb 
        # ipdb.set_trace()
        distance_cutoff_mask = distances < distance_cutoff

        edge_index = edge_index[:, distance_cutoff_mask]
        if self.atom3d:
            self.edge_attr = data.edge_attr[distance_cutoff_mask].to(self.device).view(-1, 1)
        distances = distances[distance_cutoff_mask]
        distances = distances.view(-1, 1)
    ...
    def reduce(self, x0, x, edge_index, edge_attr, batch, distances=None):
        ...
        if self.reduce_type == 'edge':
            if self.reduce_from == 'all':
                ...
                # print(f"from_ligand.all(): {from_ligand.all()}, to_ligand.all(){to_ligand.all()}")
