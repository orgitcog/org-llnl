...
from models.ggcnn import GatedGraphConv, PotentialNetAttention
...
class PotentialNetParallel(BaseModel):
    def __init__(
        self,
        in_channels,
        out_channels,
        ...
        # in_channels += 1 # Add `vdw_radii`
        super(PotentialNetParallel, self).__init__(in_channels, out_channels, loss_fn, estlabel=estlabel)
        assert (
        ...
        self.global_add_pool = global_add_pool
        self.labels = PotentialNetFullyConnected(
            non_covalent_gather_width, label_channels
        )
    ...
    def _forward(self, data, get_feature=False):
        ...
        (non_covalent_edge_index, non_covalent_edge_attr
        ...
        if not self.estLabel:
            return self.output(pool_x) # get the label too
        else:
            return self.output(pool_x), self.softmax(self.labels(pool_x))
