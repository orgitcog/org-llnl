...
class GMD(Dataset):
    ...
    def __init__(
        ...
        self.mode = mode
        # XXX(june): subset is not necessary.
        self.subset = subset
        ...
        file_path = os.path.join(data_dir, "gmd_fast2_input.h5")
        self.df = h5py.File(file_path, "r")
        comp_ids_all = list(self.df)

        self.data_list = []
        for comp_id_pose in comp_ids_all:
            pose = int(comp_id_pose.rsplit("_", 1)[-1])
            # XXX(june): do not know the GT affinity score.
            affinity = np.asarray(0.0).reshape(1, -1)
    ...
    def __getitem__(self, item):
        ...
        # TODO(june): currently not supported.
