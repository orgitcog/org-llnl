"""f90nml.namelist
   ...
   Extended by Ian Thompson, LLNL.   Jan 10, 2022
     to give toStrings outputs
     to allow for empty namelists (var_strs being an empty list)
"""
...
class Namelist(OrderedDict):
    ...
    def toString(self,toprint):
        """Output dict to a list of strings."""

        outString = []
        for grp_name, grp_vars in self.items():
            # Check for repeated namelist records (saved as lists)
            if grp_name in toprint:
                if isinstance(grp_vars, list):
                    for g_vars in grp_vars:
                        outString += [self.toString_nmlgrp(grp_name, g_vars)]
                else:
                    outString += [self.toString_nmlgrp(grp_name, grp_vars)]
        return outString

    def toString_nmlgrp(self, grp_name, grp_vars):
        """Write namelist group to str."""

        outS = '&{0}'.format(grp_name)

        for v_name, v_val in grp_vars.items():

            for v_str in self.var_strings(v_name, v_val):
                #nml_line = self.indent + '{0}'.format(v_str)
                nml_line = '{0}'.format(v_str)
                outS += ' '+nml_line
        return outS + ' /'
