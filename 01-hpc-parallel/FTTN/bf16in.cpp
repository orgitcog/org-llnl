...
int main(int argc, char** argv){
    ...
    BF16 *h_a, *h_b, *h16_c, *d16_a, *d16_b, *d16_c,
    one16=__float2bfloat16(1.), minsubnormal16 = __float2bfloat16(ldexp(1., -133)),
    minnormal16= __float2bfloat16(ldexp(1., -126)),
    ...
    h_b = new BF16[16*16];
    h_c = new FP32[16*16];
    h16_c = new BF16[16*16];

    CHECK_DEVICE_ERROR(deviceMalloc((void**)&d16_a, 16*16*sizeof(BF16)));
    CHECK_DEVICE_ERROR(deviceMalloc((void**)&d16_b, 16*16*sizeof(BF16)));
    CHECK_DEVICE_ERROR(deviceMalloc((void**)&d16_c, 16*16*sizeof(BF16)));
 ...
 /*
    ...
    */
    ...
    h_a[0] = one16;
    h_a[1] = one16;
    h_b[0] = __float2bfloat16(ldexp(1., -23) + ldexp(1., -24));
    h_b[1] = __float2bfloat16(2.);
    wmma_init_run (h_a, h_b, h_c, d16_a, d16_b, d_c, false);
    if(h_c[0] == 2.){
    ...
    */
    ...
    /*
    Test 3 extra bits under rounding to zero rounding mode
    */

    if(rtz & artz & one_extr){
        host_reset(h_a, h_b, h_c);
        h_a[0] = one16;
        h_b[0] = __float2bfloat16(1. + ldexp(1., -2));
    ...
    */
    ...
    if(three_extr & rtn & artn){
        host_reset(h_a, h_b, h_c);
        h_a[0] = __float2bfloat16(ldexp(1, -24));
        h_b[0] = one16;
        h_c[0] = 1.;
        wmma_init_run (h_a, h_b, h_c, d16_a, d16_b, d_c, false);
        if(h_c[0] == 1.){
    ...
    */
    ...
    //     h_a[index] = __float2bfloat16(ldexp(1, -24));
    //     h_b[index] = one16;
    // }
    ...
    h_a[0] = __float2bfloat16(ldexp(1, -24));
    h_b[0] = one16;
    h_a[1] = __float2bfloat16(ldexp(1, -24));
    h_b[1] = one16;
    // h_a[2] = __float2bfloat16(ldexp(1, -24));
    // h_b[2] = one16;
    h_c[0] = 1.;
    wmma_init_run(h_a, h_b, h_c, d16_a, d16_b, d_c, false);
    sum_c = h_c[0];
    ...
    for (index=0; index<2; index++) {
        h_c[0] = ldexp(1., -24);
        if (index>0){
            h_a[index-1] = __float2bfloat16(ldexp(1, -24));
            h_b[index-1] = one16;
        }
        h_a[index] = one16;
        h_b[index] = one16;
    ...
    */
    if(case1 && rtz && artz && one_extr && !stick_bit){
        ...
        h_a[0] = __float2bfloat16(ldexp(1, -24));
        h_b[0] = one16;
        h_a[1] = __float2bfloat16(ldexp(1, -25));
        h_b[1] = one16;
        h_a[2] = __float2bfloat16(ldexp(1, -25));
        h_b[2] = one16;
        h_c[0] = 1.;
        wmma_init_run (h_a, h_b, h_c, d16_a, d16_b, d_c, false);
        if(h_c[0] == 1. ){
    ...
    */
    ...
    if(case1){
        for(index = 1; index < 16; index++){
            if(index > 1){
                h_a[index-1] = __float2bfloat16(0.);
                h_b[index-1] = __float2bfloat16(0.);
                h_c[0] = 1.;
            }
            h_a[index] = __float2bfloat16(ldexp(1, -24));
            h_b[index] = one16;
    ...
    */
    ...
    if((min_preserve_uint >=4 ) & preserve_intermed & rtz&artz & one_extr &!three_extr){
        host_reset(h_a, h_b, h_c);
        h_a[0] = __float2bfloat16(ldexp(1, -23));
        h_b[0] = one16;
        h_a[1] = __float2bfloat16(ldexp(1, -25));
        h_b[1] = one16;
        h_a[2] = __float2bfloat16(ldexp(1, -25));
    ...
    else if((min_preserve_uint >=2) & preserve_intermed & rtn &artn &rtn_tie_to_even &stick_bit & three_extr){
    ...
    */
    ...
    if(norm_once & (min_preserve_uint >= 8) & one_extr &!three_extr){
        ...
        memcpy(&c_print, &smaller_c, sizeof(FP32));
        std::cout << "smaller_c = " << std::hex << c_print << std::endl;   
        h_c[0] = ldexp(1,25);
        wmma_init_run (h_a, h_b, h_c, d16_a, d16_b, d_c, false);
    ...
    /*
     ==========================  ========================== Test Rounding mode for inner product addition (bf16 output) ==========================  ========================== 
    */
#if defined(AMD)
    bool bf16_rtz=0, bf16_ru=0, bf16_rd=0, bf16_rtn=0;
    ...
    */
    ...
    h_a[1] = one16;
    h_b[0] = __float2bfloat16(2.);
    h_b[1] = __float2bfloat16(ldexp(1, -7) + ldexp(1, -8));
    wmma_init_run (h_a, h_b, h16_c, d16_a, d16_b, d16_c, false);
    if(__bfloat162float(h16_c[0]) == 2.){
        printText("inner product addition rounding down or rounding to zero.");
        bf16_rd = 1;
        bf16_rtz = 1;
    }
    else if(__bfloat162float(h16_c[0]) == 2 + ldexp(1,-6)){
        printText("inner product addition rounding up or rounding to nearest. ");
        bf16_rtn = 1;
        bf16_ru =1;
    }
        ...
        printText("Not fall in any cases, need to rethink.");               
        memcpy(&c_print, &h16_c[0], sizeof(BF16));
    ...
    */
    ...
    /*
     ========================== ==========================  Test Rounding mode for accumulation  ========================== ========================== 
    */
    bool bf16_rtza=0, bf16_rua=0, bf16_rda=0, bf16_rtna=0;
    ...
    */
     host_reset(h_a, h_b, h16_c);
    h_a[0] = one16;
    h_b[0] = __float2bfloat16(ldexp(1., -7) + ldexp(1., -8));
    h16_c[0] =__float2bfloat16(2.);
    wmma_init_run (h_a, h_b, h16_c, d16_a, d16_b, d16_c, false);
    if(__bfloat162float(h16_c[0]) == 2.){
        printText("accumulation rounding down or rounding to zero. ");
        bf16_rda = 1;
    ...
    */
    ...
    h_a[0] = one16;
    h_b[0] = __float2bfloat16(-2.);
    h16_c[0] = __float2bfloat16(-ldexp(1., -7) - ldexp(1., -8));
    wmma_init_run (h_a, h_b, h16_c, d16_a, d16_b, d16_c, false);
    if(__bfloat162float(h16_c[0]) == -2.){
        printText("accumulation rounding up or rounding to zero. ");
        bf16_rda = 0;
    ...
    /*
     ==========================  ========================== Test if bf16 output, then the full computation is in full 32 precision (at least three extra bits) ==========================  ========================== 
    */
    ...
    if(bf16_rtz & bf16_rtza){

        host_reset(h_a, h_b, h16_c);
        h_a[0] = one16;
        h_a[1] = __float2bfloat16(-ldexp(1, -23));
        h_b[0] = one16;
        h_b[1] = one16;
        wmma_init_run (h_a, h_b, h16_c, d16_a, d16_b, d16_c, false);
        if(__bfloat162float(h16_c[0]) == 1.){
            printText("if output bf16, the full computation is not performed in full32 precision and not at least three extra bits. ");
        }
        else if(__bfloat162float(h16_c[0]) == 1.- ldexp(1, -7)){
            printText("if output bf16, the full computation is performed in full32 precision, so at least three extra bits. ");
            compute_in_32 = 1;
        }
        else{
            printText("Not fall in any cases, need to rethink.");               
    ...
    else if(bf16_rtn & bf16_rtna){
        host_reset(h_a, h_b, h16_c);
        h_a[0] = one16;
        h_a[1] = one16;
        h_b[0] = one16;
        h_b[1] = __float2bfloat16(ldexp(1, -8));
        wmma_init_run (h_a, h_b, h16_c, d16_a, d16_b, d16_c, false);
        BF16 c1 = h16_c[0];
        h_b[1] = __float2bfloat16(ldexp(1, -8) + ldexp(1, -11));
        h16_c[0] = __float2bfloat16(ldexp(1, -11));
        wmma_init_run (h_a, h_b, h16_c, d16_a, d16_b, d16_c, false);
        BF16 c2 = h16_c[0];
        if(__bfloat162float(c1) != __bfloat162float(c2)){
            printText("if output bf16, the full computation is performed in full32 precision, so at least three extra bits and tie to even.");
            compute_in_32 = 1;
        }
        else if(__bfloat162float(c1) == 1.+ldexp(1, -7)){
            printText("if output bf16, the full computation is not performed in full32 precision and not at least three extra bits, or not tie to even. ");
        }
        else{
            printText("Not fall in any cases, need to rethink.");               
            memcpy(&c_print, &h16_c[0], sizeof(BF16));
    ...
    summerize(GPUModel, support_sub_input,
                ...
                min_preserve_uint,
                bf16_rtz, bf16_ru, bf16_rd, bf16_rtn);
#elif defined(NVIDIA)
    summerize(GPUModel, support_sub_input,
                rtz, ru, rd, rtn,
                artz, aru, ard, artn,
