...
#if defined(NVIDIA)
FP32 minsubnormalsp32=ldexp(1., -136);
FP32 minsubnormal32=ldexp(1., -149);
FP32 minnormal32 = ldexp(1., -126);
#elif defined(AMD)
FP32 minsubnormalsp32=ldexp(1., -149);
FP32 minsubnormal32=ldexp(1., -149);
FP32 minnormal32 = ldexp(1., -126);
#else
    #error "You should spcify the device as NVIDIA or AMD"
#endif
        // one=(1.), minsubnormal16 = (ldexp(1., -24)),
    ...
    FP32 *h_a, *h_b, *h_c, *d_a, *d_b, *d_c, minus_14 = ldexp(1., -14), one=1.,belowone= nextafterf(1., 0.);
    int index;
    ...
    h_b = new FP32[16*16];
    h_c = new FP32[16*16];
    // h16_c = new FP32[16*16];

    CHECK_DEVICE_ERROR(deviceMalloc((void**)&d_a, 16*16*sizeof(FP32)));
    CHECK_DEVICE_ERROR(deviceMalloc((void**)&d_b, 16*16*sizeof(FP32)));
    // CHECK_DEVICE_ERROR(deviceMalloc((void**)&d16_c, 16*16*sizeof(FP32)));
    ...
    */
    ...
    h_a[0] = minsubnormalsp32;
    h_b[0] = one;
    h_c[0] = 0.;
    wmma_init_run_32(h_a, h_b, h_c, d_a, d_b, d_c, false);
    if(h_c[0] == minsubnormalsp32){
        support_sub_input = 1;
...
*/
    ...
    */
    ...
    host_reset(h_a, h_b, h_c);
    h_a[0] = (1. + ldexp(1., -22) + ldexp(1., -23));
    h_b[0] = (1. + ldexp(1., -2));
    wmma_init_run_32 (h_a, h_b, h_c, d_a, d_b, d_c, false);
    if(h_c[0] == 1. + ldexp(1., -2) + ldexp(1., -22) + ldexp(1., -23)){
        printText("inner product addition rounding down or rounding to zero.");
        prd = 1;
        prtz = 1;
    }
    else if(h_c[0] ==  1. + ldexp(1., -2) + ldexp(1., -21)){
...
if(prtn){
    ...
    wmma_init_run_32 (h_a, h_b, h_c, d_a, d_b, d_c, false);
    FP32 v2=h_c[0];
    if(v1== (1. + ldexp(1., -3) + ldexp(1., -21) + ldexp(1., -22)) && 
    v2==(1. + ldexp(1., -3) + ldexp(1., -21))){
       printText("use three extra bits and round to even when tie.");
    }
    else{
        printText("Not fall in any cases, need to rethink");
        memcpy(&c_print, &v1, sizeof(FP32));
        std::cout << "v1 = " << std::hex << c_print << std::endl;
        memcpy(&c_print, &v2, sizeof(FP32));
