...
/**
 * Network entry structure from session.network
 */
 ...
 */
async function readNetworkEntries(networkPath: string): Promise<NetworkEntry[]> {
  ...
  for await (const line of rl) {
    if (line.trim()) {
      try {
        const entry = JSON.parse(line) as NetworkEntry;
        entries.push(entry);
      } catch {
 ...
 */
export async function generateNetworkSummary(networkPath: string): Promise<string> {
  ...
  const byType = new Map<string, ResourceTypeStats>();
  for (const entry of entries) {
    ...
    if (entry.fromCache) existing.cached++;
  ...
  const entriesWithDetailedTiming = entries.filter(e =>
    e.timing && (e.timing.dns || e.timing.connect || e.timing.ttfb)
  );
  ...
  if (entriesWithDetailedTiming.length > 10) {
    ...
    lines.push('|-------|------------------|');
    if (avgDns > 0) lines.push(`| DNS Lookup | ${formatDuration(avgDns)} |`);
    if (avgConnect > 0) lines.push(`| Connection | ${formatDuration(avgConnect)} |`);
    if (avgTtfb > 0) lines.push(`| Time to First Byte | ${formatDuration(avgTtfb)} |`);
    if (avgDownload > 0) lines.push(`| Download | ${formatDuration(avgDownload)} |`);
    lines.push('');
  }
 ...
 */
export async function generateNetworkSummaryFile(sessionDir: string): Promise<string | null> {
