...
cudaError_t gpuMalloc(void** ptr, size_t bytes)
{
...
cudaError_t gpuMemcpy(void* dst, const void* src, size_t bytes, cudaMemcpyKind kind)
{
...
{
	T* t;
	checkCudaErrors(gpuMalloc((void**)&t, sizeof(T)*N));
	return t;
}
...
{
	checkCudaErrors(gpuFree((void*)array));
}

template <typename T>
void copy(T* dst, T* src, int N)
{
	checkCudaErrors(gpuMemcpy((void*)dst, (void*)src, N*sizeof(T), cudaMemcpyDeviceToDevice));
}

template <typename T>
void upload(T* dst, T* src, int N)
{
	checkCudaErrors(gpuMemcpy((void*)dst, (void*)src, N*sizeof(T), cudaMemcpyHostToDevice));
}

template <typename T>
void download(T* dst, T* src, int N)
{
	checkCudaErrors(gpuMemcpy((void*)dst, (void*)src, N*sizeof(T), cudaMemcpyDeviceToHost));
}
...
int main(int argc, char** argv)
{
  ...
  //~ #pragma xpl diagnostic tracePrint(std::cerr; areas, elements_surrounding_elements, normals, old_variables, variables, fluxes, step_factors)
	std::cout << (sdkGetAverageTimerValue(&timer)/1000.0)  / NUM_ITER << " seconds per iteration" << std::endl;

  std::cerr << static_cast<const void*>(fluxes) << ", " 
            << static_cast<const void*>(step_factors)
            << std::endl;
