...
def geoGetPointsOfCenteredBox(cyx, sy, sx=None, clockwise=False):
    """
    return list of 4 pt-pairs
    4 corners of box centered at `cyx`,
    side lengths are `sy`, `sx` 
    if `sx` is None, it defaults to `sy`

    if clockwise: 2nd and 4th point will be swapped (-> mathematical negatve order)
    """
    if sx is None:
        sx=sy
    sx2=sx/2
    ...
    x1=x0+sx
    if clockwise:
        return [(y0,x0),(y1,x0),(y1,x1),(y0,x1)]
    else:
        return [(y0,x0),(y0,x1),(y1,x1),(y1,x0)]


def geoGetPointsOfBox(yx0,yx1, clockwise=False):
    """
    return list of 4 pt-pairs
    4 corners of box with corners yx0, yx1
    side lengths are `sy`, `sx` 
    if `sx` is None, it defaults to `sy`
