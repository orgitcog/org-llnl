...
ID_AUTOCOMP_SORTCASESENSITIVE = wx.NewId()
...
ID_WRAP = wx.NewId()
ID_DISPLAYHOOK_NDARRAY = wx.NewId()
ID_SHOWCOMPLEXVALS = wx.NewId()
ID_CLEARMODULESCACHE = wx.NewId()
ID_AUTOSAVE_COMMANDWISE = wx.NewId()
ID_USEAA = wx.NewId()
ID_MARKWINDOWS = wx.NewId()
ID_CLEARCLOSEDVIEWERS = wx.NewId()
ID_RAISEWINDOWS = wx.NewId()
ID_MINIMIZEWINDOWS = wx.NewId()
ID_CLOSEALLWINDOWS = wx.NewId()
ID_TEST5 = wx.NewId()
ID_TEST2 = wx.NewId()
ID_TESTALL = wx.NewId()
...
ID_VSYNCVIEWERS= wx.NewId()
ID_VPLOTSLIDER   = wx.NewId()
ID_VPLOTINSPECT  = wx.NewId()
ID_OSXBUG = wx.NewId()
...
class Frame(wx.Frame):
    ...
    def __createMenus(self):
        ...
        m.Append(ID_AUTOCOMP_SORTCASESENSITIVE, "Sort Case Sensitive",
                 "Don't ignore case when sorting attributes", wx.ITEM_CHECK)
        ...
        m.Append(ID_DISPLAYHOOK_NDARRAY, "&Display ndArray popup",
                 "Show popup menu when interactive result is an ndArray", wx.ITEM_CHECK)
        m.Append(ID_SHOWCOMPLEXVALS, "Show Comple&x Vals (real,imag)",
                 "Show complex values as real,imag (not abs) in viewer for mouse over", wx.ITEM_CHECK)
        m.Append(ID_CLEARMODULESCACHE, "Clear import-completion cache",
                 "Clear cache and make import <TAB> regenerate list of available modules")
        m.Append(ID_AUTOSAVE_COMMANDWISE, "Auto &Save every command\tCtrl-Shift-S",
                 "Call saveSession automatically after every shell command", wx.ITEM_CHECK)

        m = self.priithonMenu = wx.Menu()
        m.Append(ID_VANIMATE, "viewer &animation control",
                 "Open viewer animation control -- Y.vAnimate()")
        m.Append(ID_VSYNCVIEWERS, "&sync viewers gui",
                 "Open gui to set cross-viewer syncronisation -- Y.vSyncViewersGui()")
        m.Append(ID_VPLOTINSPECT, "&inspect plot data points",
                 "Open GUI to inspect datapoints of datasets shown in plot figure -- Y.vPlotInspectDatapointsGUI()")
        m.Append(ID_VPLOTSLIDER, "use &plot figure as z-slider",
                 "Open GUI to make a plot figure function as z slider for a viewer -- Y.vPlotAsSliderGUI()")
        m.Append(ID_CLEARCLOSEDVIEWERS, "&Clear unused high viewer IDs", 
                 "delete trailing closed Y.viewers items -- Y.vClearClosedViewers()")
        m.Append(ID_RAISEWINDOWS, "&Raise all Windows", 
                 "raise all Priithon windows above others")
        m.Append(ID_MINIMIZEWINDOWS, "Mi&nimize all Windows", 
        ...
        m.Append(ID_CLOSEALLWINDOWS, "C&lose all viewer windows...",
                 "close all open viewer windows and clear viewer IDs & Y.viewers items")
        m.AppendSeparator()
        m.Append(ID_TEST5, "show mandel&brot set", 
                 "demonstrate colorful single channel image [ Y.test5() ]")
        m.Append(ID_TEST2, "test multi c&olor viewer [ Y.test2() ]", 
                 "test multi color viewer [ Y.test2() ]")
        m.Append(ID_TESTALL, "do all &tests [ Y.test...() ]", 
                 "call all Y.test[1,2,3,...] functions")

        m = self.helpMenu = wx.Menu()
        m.Append(ID_HELP1, "&Shell help", "Helpful info about Priihon's command shell")
        m.Append(ID_HELP3, "&Viewer help", "Helpful info about viewer shortcuts")
        m.Append(ID_HELP2, "&Online modules documentation", "Open web browser with Priihon epydoc page at googlecode")
        ...
        wx.EVT_MENU(self, ID_VSYNCVIEWERS, self.OnSynvViewersGui)
        wx.EVT_MENU(self, ID_VPLOTSLIDER, self.OnVplotslider)
        wx.EVT_MENU(self, ID_VPLOTINSPECT, self.OnVplotInspect)
        ...
        wx.EVT_MENU(self, ID_HELP1, self.OnHelp1)
        wx.EVT_MENU(self, ID_HELP2, self.OnHelp2)
        wx.EVT_MENU(self, ID_HELP3, self.OnHelp3)
        ...
        wx.EVT_MENU(self, ID_AUTOCOMP_SORTCASESENSITIVE, self.OnAutoCompleteSortCase)
        ...
        wx.EVT_MENU(self, ID_DISPLAYHOOK_NDARRAY, self.OnDisplayHookNdArray)
        wx.EVT_MENU(self, ID_SHOWCOMPLEXVALS, self.OnShowComplexVals)
        wx.EVT_MENU(self, ID_CLEARMODULESCACHE, self.OnClearModulesCache)
        wx.EVT_MENU(self, ID_AUTOSAVE_COMMANDWISE, self.OnAutosaveCommandwise)
        wx.EVT_MENU(self, ID_MARKWINDOWS, self.OnMarkWindows)
        wx.EVT_MENU(self, ID_CLEARCLOSEDVIEWERS, self.OnClearClosedViewers)
        wx.EVT_MENU(self, ID_RAISEWINDOWS, self.OnRaiseWindows)
        wx.EVT_MENU(self, ID_MINIMIZEWINDOWS, self.OnMinimizeWindows)
        wx.EVT_MENU(self, ID_CLOSEALLWINDOWS, self.OnCloseAllWindows)
        ...
        wx.EVT_UPDATE_UI(self, ID_DISPLAYHOOK_NDARRAY, self.OnUpdateMenu)
        wx.EVT_UPDATE_UI(self, ID_SHOWCOMPLEXVALS, self.OnUpdateMenu)
        wx.EVT_UPDATE_UI(self, ID_AUTOSAVE_COMMANDWISE, self.OnUpdateMenu)
    ...
    def OnVanimate(self, evt=None):
        from ..all import Y
        Y.vAnimate()
    def OnSynvViewersGui(self, evt=None):
        from ..all import Y
        Y.vSyncViewersGui()
    ...
    def OnVplotInspect(self, evt=None):
        from ..all import Y
        Y.vPlotInspectDatapointsGUI()
...
#         Y.listArrayViewer()
    ...
    def OnHelp1(self, event):
        """Display helpTxt in Gui window."""
        from ..all import Y
        from .. import PriDocs
        
        Y.showHtmlInfo(PriDocs.priShell_helpHTML,
                       title="Priithon shell help",
                       size=(700,800))


    def OnHelp2(self, event):
        """Open WebBrowser with epydoc"""
        import webbrowser
        webbrowser.open('http://priithon.googlecode.com/hg/priithon_docs/PriithonEpydoc/frames.html')


    def OnHelp3(self, event):
        """Display helpfu info about viewer window shortcuts."""
        from ..all import Y
        from .. import PriDocs
        Y.showHtmlInfo(PriDocs.viewerHelp,
                       title="Priithon viewer help",
                       size=(700,800))
    ...
    def OnAutoCompleteSortCase(self, event):
        win = wx.Window_FindFocus()
        win.autoCompleteSortCaseSensitive = event.IsChecked()
    ...
    def OnDisplayHookNdArray(self, event):
        from .. import PriConfig
        PriConfig.shellDisplayHookNdarrayPopup = event.IsChecked()

    def OnClearModulesCache(self, event):
        from .. import useful as U
        U._getModules_cache.clear()

    def OnShowComplexVals(self, event):
        from .. import PriConfig
        PriConfig.viewerShowComplexVals = event.IsChecked()

    def OnAutosaveCommandwise(self, event):
        from .. import PriConfig
        PriConfig.autoSaveEveryCommand = event.IsChecked()
        if PriConfig.autoSaveEveryCommand:
            self.SetStatusText("autosaving after every command turned on.")
        else:
            self.SetStatusText("autosaving after every command turned off.")


    def OnClearClosedViewers(self, event):
        from ..all import Y
        Y.vClearClosedViewers()
    ...
    def OnCloseAllWindows(self, event):
        r = wx.MessageBox("are you sure to close all viewers?", 
                          "close all viewers?", wx.YES_NO)
        if r == wx.YES:
            from ..all import Y
            Y.vClose(vid="all")
            Y.vClearClosedViewers()
    ...
    def OnTest5(self, event):
        from ..all import Y
        Y.test5()
        self.shell.addHistory("Y.test5()")
    def OnTest2(self, event):
        from ..all import Y
        Y.test2()
        self.shell.addHistory("Y.test2()")
    def OnTestAll(self, event):
        from ..all import Y
        for i in range(1,7):
            exec "Y.test%d()"%(i,)
        self.shell.addHistory("""for i in range(1,7):\n    exec "Y.test%d()"%(i,)""")
    ...
    def OnUpdateMenu(self, event):
        ...
        try:
            ...
            elif id == ID_AUTOCOMP_SORTCASESENSITIVE:
                event.Check(win.autoCompleteSortCaseSensitive)
            ...
            elif id == ID_DISPLAYHOOK_NDARRAY:
                from ..PriConfig import shellDisplayHookNdarrayPopup
                event.Check(shellDisplayHookNdarrayPopup)
            elif id == ID_SHOWCOMPLEXVALS:
                from ..PriConfig import viewerShowComplexVals
                event.Check(viewerShowComplexVals)
            elif id == ID_AUTOSAVE_COMMANDWISE:
                from ..PriConfig import autoSaveEveryCommand
                event.Check(autoSaveEveryCommand)
            elif id == ID_OSXBUG:
                from ..viewer import bugXiGraphics
