...
import wx
#20100928 from colormap     import colormap_map
...
class axis_window(wx.Window,axis_object):
    ...
    def format_popup(self,pos):
        ...
        menu.Append(600, 'Change Font', 'Change Text Font')
        wx.EVT_MENU(self, 600, self.OnFont)
        menu.Append(602, 'Change Format', 'Change Number Format')
        wx.EVT_MENU(self, 602, self.OnFormat)
        menu.UpdateUI()
        self.PopupMenuXY(menu,pos[0],pos[1])
    ...
    def OnFormat(self,event):
        #print 'on text:',self.text
        dlg = wx.TextEntryDialog(self.GetParent(), 'change tick number format',
                            defaultValue="%d")
        if dlg.ShowModal() == wx.ID_OK:
            format=dlg.GetValue()
            self.tickFormatter = lambda tickVal: format%(tickVal,)
            self.plot_canvas.update()     
        dlg.Destroy()
...
class image_object(property_object):
    ...
    def form_image(self):
        ...
        if type(self.colormap) == type(''):
            from colormap     import colormap_map #20100928
            try:
