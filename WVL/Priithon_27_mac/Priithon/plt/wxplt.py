...
class plot_canvas(wx.Window,property_object):
    ...
    def on_removeDataset(self, event):
        dataset = event.GetId() - 600
        from Priithon.all import Y
        # CHECK if GetParent() is ok here ?!
        Y.plotDatasetRemove(dataset, self.GetParent())

    def on_hold(self, event):
        # 20110808-changed self.GetParent().hold to self.hold
        self.hold = event.IsChecked() and "yes" or "no"
    def on_inspectGUI(self, event):
        from Priithon.all import Y
        Y.vPlotInspectDatapointsGUI(self.GetParent())

    def on_vPlotslider(self, event):
        from Priithon.all import Y
        Y.vPlotAsSliderGUI(figureNo=self.GetParent())
    ...
    def format_popup(self,pos):
        menuSubRemoveDataset = wx.Menu()
        numClientLineList = len(self.line_list)
        for i in range(numClientLineList-1,-1,-1):
            ll = self.line_list[i]
            if i == numClientLineList-1:
                ss = "%s (latest) [%s]" % (i, ll.line.color)
            else:
                ss = "%s [%s]" % (i, ll.line.color)
            mi = menuSubRemoveDataset.Append(600+i, ss)
            #mi.
            wx.EVT_MENU(self, 600+i,self.on_removeDataset) 
        ...
        menu.AppendMenu(512, 'remove dataset', menuSubRemoveDataset, 'remove existing dataset in plot graph, reuse color when removing latest')
        menu.AppendCheckItem(509, 'hold plot', "don't clear before next plot")
        menu.Append(510, 'inspect data points', 'open data point inspect GUI -- Y.vPlotInspectDatapointGUI()')
        menu.Append(511, 'use plot as z-slider', 'Open GUI to make a plot figure function as z slider for a viewer -- Y.vPlotAsSliderGUI()')
        ...
        # 20110808-changed self.GetParent().hold to self.hold  -- CHECK if GetParent() is ok here ?!
        menu.Check(509, self.hold in ['on','yes'])
        wx.EVT_MENU(self, 510, self.on_inspectGUI)
        wx.EVT_MENU(self, 511, self.on_vPlotslider)
    ...
    def save_csv(self, path, csv_sep='\t', transpose=False):
        """
        save all datasets into file
        as comma-separated-values (CSV)
        use `csv_sep` instead om "comma" (default '\t')
        if `transpose`: save each dataset into two (x,y) columns, rather than two rows
            (`transpose` = False might be faster)

        `path` can also be an already open file-like object
        """
    ...
    def save_csv_singleXcolumn(self, path, csv_sep='\t'):
        """
        save all datasets into file
        as comma-separated-values (CSV)
        use `csv_sep` instead om "comma" (default '\t')
        all y,x pairs across all datasets are compacted to share one x-column
        ...
        nYYS = len(yys)
        for x in sorted(xs):
            f.write(str(x)+csv_sep)
            for iy,ys in enumerate(yys):
                try:
                    f.write(str(ys[x]))
                except KeyError:
                    pass
...
class plot_frame(wx.Frame):
    ...
    def __init__(self, parent=None, id = -1, title = '', 
        ...
        menu = wx.Menu()
        menu.Append(200, '&Save as Image...', 'Save plot to image file')
        wx.EVT_MENU(self, 200, self.file_save_as)
        menu.Append(202, '&Save as CSV (column order)...', 'Save plot to CSV text file - tab separated, each dataset goes into two(x,y) columns')
        wx.EVT_MENU(self, 202, lambda ev: self.file_save_csv(ev, transpose=True))
        menu.Append(201, '&Save as CSV (row order)...', 'Save plot to CSV text file - tab separated, each dataset goes into two (x,y) rows')
        wx.EVT_MENU(self, 201, self.file_save_csv)
        menu.Append(208, '&Save as CSV (column order, merge to one X col)...', 'Save plot to CSV text file - tab separated, each line as (x,y1,y2,y3)')
        wx.EVT_MENU(self, 208, self.file_save_csv_singleXcolumn)

        menu.Append(207, 'Copy CSV to clipboard (column order)...', 'Copy CSV into clipboard - tab separated, each dataset goes into two(x,y) columns')
        wx.EVT_MENU(self, 207, lambda ev: self.file_copy_csv(ev, transpose=True))
        menu.Append(206, '&Copy CSV to clipboard (row order)...', 'Copy CSV into clipboard - tab separated, each dataset goes into two (x,y) rows')
        wx.EVT_MENU(self, 206, self.file_copy_csv)
        menu.Append(209, '&Copy CSV (column order, merge to one X col)...', 'Copy plot to CSV into clipboard - tab separated, each line as (x,y1,y2,y3)')
        wx.EVT_MENU(self, 209, self.file_copy_csv_singleXcolumn)
        ...
        #20100928 self.print_data = wx.PrintData()
        self.print_data = None #20100928: before was apparently inspired by http://wiki.wxpython.org/Printing
        #20100928 self.print_data.SetPaperId(wx.PAPER_LETTER)
    ...
    def file_save_as(self, event):
        ...
        wildcard = "PNG files (*.png)|*.png|" \
                   ...
                   "TIFF files (*.tif)|*.tif|" \
                   "All Files |*"
        dlg = wx.FileDialog(self, "Save As Image", ".", "", wildcard, wx.SAVE)
    ...
    def file_copy_csv(self, event, csv_sep='\t', transpose=False):
        import StringIO
        s=StringIO.StringIO()
        self.client.save_csv(s, csv_sep, transpose)

        from Priithon import usefulX
        usefulX.clipboardSetText(s.getvalue())
    ...
    def file_copy_csv_singleXcolumn(self, event, csv_sep='\t'):
        import StringIO
        s=StringIO.StringIO()
        self.client.save_csv_singleXcolumn(s, csv_sep)

        from Priithon import usefulX
        usefulX.clipboardSetText(s.getvalue())

    def file_save_csv_singleXcolumn(self, event, csv_sep='\t'):
        import os
        wildcard = "CSV files (*.csv)|*.csv|" \
                   "TSV files (*.tsv)|*.tsv|" \
                   "TXT files (*.txt)|*.txt|" \
                   "All Files |*"
        dlg = wx.FileDialog(self, "Save single X column CSV As", ".", "", wildcard, wx.SAVE|wx.FD_OVERWRITE_PROMPT)
        if dlg.ShowModal() == wx.ID_OK and dlg.GetPath():
            self.client.save_csv_singleXcolumn(dlg.GetPath(), csv_sep)
        dlg.Destroy()
    ...
    # 20110808  -- this is a HACK - removed !!
    # def update(self):
    #     self.client.update()
