...
class DispatcherServer(asyncore.dispatcher): #  fails w/2.5  _with_send):
...
class MainWindow(wx.Frame):
    def __init__(self, host, port, threaded=0):
        wx.Frame.__init__(self, None, title="Demo echo server on %s:%s"%(host,port))
        ...
        self.ds = DispatcherServer(host, port, LineEchoConnection, self.LogString) #seb self)
...
def socketServerStart(onRead,
    ...
    factory = new.classobj("Custom_DispatcherConnection", 
                           ...
                           handlerFunc_dict)
    ds = DispatcherServer(host, port, factory, logFcn)
...
def socketServer_demo(port=4711, host='localhost', threaded=False, ret=False):
