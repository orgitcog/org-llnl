...
def magicArr(n, dtype=N.int):
    ...
    if n < 0 or n == 2:                    # consistent with Octave
        raise ValueError("No such magic square exist.")
    elif n%2 == 1:
        m = n>>1
        b = n*n + 1
        #seb _constant = n*b>>1
        return (N.tile(N.arange(1,b,n, dtype=dtype),n+2)[m:-m-1].reshape(n,n+1)[...,1:]+
              N.tile(N.arange(n, dtype=dtype),n+2).reshape(n,n+2)[...,1:-1]).transpose()
    elif n%4 == 0:
        b = n*n + 1
        _constant = n*b>>1
        d=N.arange(1, b, dtype=dtype).reshape(n, n)
        ...
        k = m>>1
        b = m*m
        d = N.tile(magicArr(m, dtype=dtype), (2,2))          # that changes the _constant
        #seb _constant = _constant*8 - n - m     
        d[:m, :k] += 3*b
        d[m:,k:m] += 3*b
        d[ k,  k] += 3*b
        d[ k,  0] -= 3*b
...
def magicArr_check(arr):
    ...
    if a.ndim == 2:
        m = N.flipud(a).trace()
        t = (N.r_[a.sum(axis=0), a.sum(axis=1), a.trace()] == m).all()
        if t == True:        # not "is" because t is a NumPy boolean  
            #seb _constant = m
            return m #True        # seb original version: not "return t",
        else:                     # seb original version: to make sure that
            return False          # seb original version: the return value is of the bool type     
    elif a.ndim == 1:
        s = N.sqrt(a.size)
        if a.size == s*s:
            return magicArr_test(a.reshape(s,s))

    raise ValueError, "arr must be a 2d array or 1d with a square length"
