...
class ndarray_inAndorSifFile(N.ndarray):
    def __new__(cls, input_array, andorSifInfo=None):
        obj = N.asanyarray(input_array).view(cls)
        obj.AndorSIF = mrcInfo
        return obj
...
def _data_withAndorSIF(info):
    ...
    data.AndorSIF = info
    '''
    data = ndarray_inAndorSifFile(info.imageData, info)
    data.AndorSIF.imageData = weakref.proxy( info.imageData ) # remove circular reference
...
"""%s.__dict__
    ...
    shape2d=tuple((1 + N.diff(info.frameArea, axis=0))[0][::-1]//info.frameBins)
