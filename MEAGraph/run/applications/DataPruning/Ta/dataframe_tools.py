# Minor modifications on https://github.com/FitSNAP/FitSNAP/blob/master/fitsnap3lib/tools/dataframe_tools.py
import numpy as np
import pandas as pd
...
class DataframeTools():
    ...
    def plot_tsne(self, num_descriptors,new_df, point_size=0.05):
        from sklearn.manifold import TSNE
        from matplotlib.lines import Line2D 
        tsne = TSNE()
        numeric_df = new_df.iloc[:,0:num_descriptors+1]
       # numeric_df = self.df.iloc[:,0:num_descriptors+1] # whatever your number of A matrix columns is
        label_df = new_df['Groups']
    ...
    def snap_descriptors(self, num_descriptors):
        numeric_df = self.df.iloc[:,0:num_descriptors+1]
        label_df = self.df['Groups']
        X = numeric_df.to_numpy()
        return X,label_df
    def count_natoms(self):
        row_type=self.df.loc[:,"Row_Type"]
        nconfigs = row_type.tolist().count("Energy")
        natoms_per_config = np.zeros(nconfigs).astype(int)
        config_indx = -1
