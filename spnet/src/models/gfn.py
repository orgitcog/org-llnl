...
class SceneEmbeddingHead(nn.Module):
    def __init__(self, reid_head, emb_head, featmap_name='feat_res3',
            pool_size=56):
        super().__init__()
        # Pooling layer
        self.image_roi_pool = ScenePool(featmap_name=featmap_name,
        ...
        ## Set norms for the reid embedding head to None
        self.image_emb_head.lut_norm = None
        self.image_emb_head.norm_type = None

    def forward(self, x: Dict[str, Tensor]):
        x = self.image_roi_pool(x)
        x = self.image_reid_head(x)
        # Old: self.image_emb_head(x)[0]
        x = self.image_emb_head(x)
        return x
...
# GroupNorm clone which handles different input shapes
class GroupNorm(nn.Module):
    def __init__(self, d):
        super().__init__()
        self.norm = nn.GroupNorm(num_groups=1, num_channels=d)
...
class QueryImageFuser(nn.Module):
    def __init__(self, d, gfn_query_mode=None,
            gfn_activation_mode='se', se_temp=0.2, gfn_norm_type='batchnorm'):
        super().__init__()
        ...
        if self.gfn_activation_mode != 'identity':
            if gfn_norm_type in (None, 'lutnorm'):
                self.norm = nn.Identity()
            elif gfn_norm_type == 'batchnorm':
                self.norm = BatchNorm1d(d)
            elif gfn_norm_type == 'groupnorm':
                self.norm = GroupNorm(d)
...
class GalleryFilterNetwork(nn.Module):
    def __init__(self, roi_pool, reid_head, emb_head,
            ...
            gfn_activation_mode='se', gfn_scene_pool_size=56, gfn_norm_type='batchnorm',
            pos_num_sample=1, neg_num_sample=1,
            reid_loss=None, featmap_name='feat_res3',
            fuse_batch_size=32, mode='combined', use_image_lut=False):
        super().__init__()
        self.gfn_query_mode = gfn_query_mode
        self.query_roi_pool = roi_pool
        self.query_reid_head = reid_head
        self.query_emb_head = emb_head
        self.gfn_norm_type = gfn_norm_type
        self.head = SceneEmbeddingHead(reid_head, emb_head,
            featmap_name=featmap_name,
            pool_size=gfn_scene_pool_size)
        self.gfn_mode = mode
        if self.gfn_mode not in  ('image', 'separate'):
            self.fuser = QueryImageFuser(emb_dim,
                gfn_query_mode=gfn_query_mode, gfn_activation_mode=gfn_activation_mode, se_temp=se_temp, gfn_norm_type=gfn_norm_type)
        ...
        # Norm layer
        if self.gfn_norm_type == 'lutnorm':
            self.scene_norm = SafeBatchNorm1d(emb_dim)
    ...
    def get_scene_emb(self, features: Dict[str, Tensor]) -> Tensor:
        ...
        if self.gfn_norm_type == 'lutnorm':
            if self.training:
                self.scene_norm.eval()
                scene_emb = self.scene_norm(scene_emb)
                self.scene_norm.train()
            else:
                scene_emb = self.scene_norm(scene_emb)
        # return scene emb
        return scene_emb
    ...
    def forward(self, features, targets, image_shapes):
        ...
        image_labels = torch.LongTensor(gfn_image_labels).to(device)
        gt_boxes = [t["boxes"].to(torch.float) if len(t['boxes']) > 0 else torch.empty(0, 4, dtype=torch.float, device=device) for t in targets]
        ...
        gfn_met_label_tsr = torch.LongTensor(met_num_list).to(device)
        # Image indices
        ...
        # Prep known tsr
        gfn_met_known_tsr = torch.BoolTensor([is_known_dict[l] for l in gfn_met_label_tsr.tolist()]).to(device)
        ...
        # XXX: old return is query_emb, _
        query_emb = self.query_emb_head(query_features)
        Q, D = query_emb.size()
        if Q == 0:
            gfn_loss = torch.tensor(0.0).to(device)
            gfn_loss_dict = {'gfn_loss': gfn_loss}
            gfn_metric_dict = {}
            return gfn_loss_dict, gfn_metric_dict

        # Extract image features
        scene_emb = self.get_scene_emb(features)
        I = scene_emb.size(0)

        # Additional label prep following (potential)
        gfn_image_idx_tsr = torch.LongTensor(gfn_image_indices).to(device)
        ...
        if self.use_image_lut:
                ...
                for rand_idx in image_rand_idx:
                    ...
                    if query_label in image_query_label_set:
                        if (image_id not in chosen_image_id_set) and (image_id not in image_id_list): 
                            #
                            lut_image_feat_list.append(image_dict['image_feat'].to(device))
                            lut_query_feat_list.append(image_dict['query_feat'].to(device)) 
                            lut_query_label_list.append(image_dict['query_label'].to(device)) 
                            lut_query_known_list.append(image_dict['query_known'].to(device)) 
                            lut_lut_label_list.append(image_dict['lut_label'].to(device)) 
                            lut_query_label_set_list.append(image_dict['query_label_set']) 
                            ## This assumes nothing is discarded!!!
                            lut_query_image_idx_list.append(torch.LongTensor([chosen_idx]*len(image_dict['query_feat'])).to(device))
                            chosen_idx += 1
            ...
            if True:
                if len(lut_image_feat_list) > 0:
                    lut_scene_emb = torch.cat(lut_image_feat_list, dim=0)
                    ### normalize lut feats
                    if self.gfn_norm_type == 'lutnorm':
                        lut_scene_emb = self.scene_norm(lut_scene_emb)
                else:
                    lut_scene_emb = torch.empty((0, scene_emb.shape[1])).to(scene_emb)
                scene_emb = torch.cat([scene_emb, lut_scene_emb], dim=0)
            else:
        ...
        if self.gfn_query_mode == 'oim':
            ...
            #
            oim_emb = query_emb
            oim_emb[gfn_lut_pid_mask] = self.reid_loss.lut[gfn_lut_label_tsr[gfn_lut_pid_mask]].to(device)
            oim_nonzero_mask = ~((oim_emb==0).all(dim=1))
            gfn_lut_pid_mask = gfn_lut_pid_mask & oim_nonzero_mask
            query_emb[gfn_lut_pid_mask] = self.reid_loss.lut[gfn_lut_label_tsr[gfn_lut_pid_mask]].to(device)
            #
        ...
        ## Combine query features with the image against which they are to be compared
            ...
            identity_mask = torch.zeros(I, Q, dtype=torch.bool).to(device)
        ...
        gfn_image_pid_tsr = -torch.ones(len(gfn_image_sets), gfn_max_len, dtype=torch.long).to(device)
        _gfn_image_pid_mask = torch.zeros(len(gfn_image_sets), gfn_max_len, dtype=torch.bool).to(device)
        ...
        gfn_image_pid_tsr[empty_mask] = (-torch.arange(1, 1+empty_mask.sum().item())).to(device)
