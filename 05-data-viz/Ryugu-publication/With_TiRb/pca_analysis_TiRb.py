...
In this case, PCA is only done on the Chondrite data, with Ti/Rb added as
per Greg Brennecka's email sent 08/14
"""
...
if __name__ == "__main__":
    ...
    for idx, isotope in enumerate(col_order):
        if isotope == "Ti/Rb":
            twosd = df.loc[:, didx[isotope, "2SD"]]
            twose = (
                0.5 * twosd
            )  # Approximating 2SE as 2SD / 2 was determined to be acceptable for this analysis
            df.insert(
    ...
    # Drop all non-chondrites
    df.drop(labels=["NC"], axis=0, level=0, inplace=True)

    # Drop isotopes and NaNs
    # Anything dropped here won't be used in any of the subsequent analysis
    df = (
        df.drop(iso_drop_list, axis=1, level=0)
        .dropna(
            axis=1, how="all"
        )  # Drop the Ti/Rb columns with all NaN. Must be done before next line
        .dropna(axis=0, how="any")  # Drop rows with NaN
    )
    ...
    save_suffix = "_" + ".".join(list(dfm)).replace("Ti/Rb", "Ti-RB")
    fig.savefig(save_name + save_suffix + ".png")

    # Set up file name
    save_name = "asteroid_pca"
    if "Ti/Rb" in list(dfm):
        plot_title = "C"
