 ...
 *
 * SEP-19 migrated to C++ (Peter Pirkelbauer)
 ******************************************************************
 */
...
float *alloc_1d_dbl(int n)
{
  ...
  allocated = (float *) malloc ((unsigned) (n * sizeof (float)));
  if (allocated == NULL) {
...
float** alloc_2d_dbl(int m, int n)
{
  ...
  allocated = (float **) malloc ((unsigned) (m * sizeof (float *)));
  if (allocated == NULL) {
...
BPNN *bpnn_read(char *filename)
{
  ...
  allocated = bpnn_internal_create(n1, n2, n3);
  ...
  for (i = 0; i <= n1; i++) {
    for (j = 0; j <= n2; j++) {
      fastcopy(&(allocated->input_weights[i][j]), &mem[memcnt], sizeof(float));
      memcnt += sizeof(float);
    }
  }
  free(mem);
  ...
  for (i = 0; i <= n2; i++) {
    for (j = 0; j <= n3; j++) {
      fastcopy(&(allocated->hidden_weights[i][j]), &mem[memcnt], sizeof(float));
      memcnt += sizeof(float);
    }
  }
  free(mem);
  ...
  bpnn_zero_weights(allocated->input_prev_weights, n1, n2);
  bpnn_zero_weights(allocated->hidden_prev_weights, n2, n3);
