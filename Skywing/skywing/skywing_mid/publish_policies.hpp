...
{
public:
    PublishOnLinfShift(scalar_t thresh) : linf_threshold_(thresh) {}
...
{
public:
    PublishOnRatioShift(scalar_t thresh) : shift_threshold_(thresh) {}

    template <typename ValueType>
    bool operator()(const ValueType& new_vals, const ValueType& old_vals)
    {
        scalar_t new_ratio =
            std::get<ind1>(new_vals) / std::get<ind2>(new_vals);
        scalar_t old_ratio =
            std::get<ind1>(old_vals) / std::get<ind2>(old_vals);
        return (new_ratio - old_ratio) > shift_threshold_
               || (old_ratio - new_ratio) > shift_threshold_;
    }
