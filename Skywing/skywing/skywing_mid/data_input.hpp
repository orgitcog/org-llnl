...
/**
 * This function inputs a vector or matrix from an unstructured and dense matrix
 * market format. In the future, this functionality may be expanded to other
 * types and formats.
 */

template <typename T>
std::vector<std::vector<T>>
input_matrix_from_matrix_market(std::string directory, std::string matrix_name)
{
    ...
    // This extra resize loop is due to how matrix market stacks columns on top
    // of each other.
...
template <typename T>
std::vector<T> input_vector_from_matrix_market(std::string directory,
                                               std::string vector_name)
{
    ...
    if (input_rows == 1 || input_cols == 1) {
        ...
        std::cerr << "WARNING: This is not a vector: row number: " << input_rows
                  << "col number: " << input_cols << std::endl;
    }
    fin.close();
    return return_vec;
}
